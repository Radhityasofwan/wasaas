...
// SSE/EventSource cannot set headers, so allow ?apiKey=...
export async function apiKeyAuth(req: any, res: any, next: any) {
  const rawHeader = String(req.header("x-api-key") || "").trim();
  const rawQuery = String(req.query?.apiKey || "").trim();
  const raw = rawHeader || rawQuery;

  if (!raw) return res.status(401).json({ ok: false, error: "missing x-api-key (or apiKey query param)" });
