...
// === 1. FITUR BARU: Get Detail untuk UI ===
...
export async function getBroadcastItems(jobId: number, tenantId: number, limit = 100, offset = 0) {
  // Mengambil detail pesan dan status per nomor
  const [rows] = await pool.query<any[]>(
...
// === 2. FITUR BARU: Bulk Delete (Hapus Job + Items) ===
export async function deleteBroadcastJob(jobId: number, tenantId: number) {
  // Hapus items dulu (best practice jika tidak ada CASCADE di DB)
  await pool.query(
...
// === Helper untuk Webhook (Reply Tracking) ===
export async function markBroadcastReply(originalMessageId: string, fromNumber: string) {
  // Cari item broadcast yang memiliki wa_message_id sesuai dengan yang di-reply user
  // DAN nomor pengirim cocok (security check)
  const [res] = await pool.query<any>(
...
export async function handleBroadcastReply(tenantId: number, from: string, textBody: string, quotedMessageId: string | null) {
  ...
  if (isReply) {
    ...
    // Kirim webhook event 'broadcast.reply'
...
export async function processBroadcastQueue() {
  ...
  try {
    ...
    if (result.ok) {
      // SUCCESS: Simpan WA Message ID agar bisa dilacak reply-nya nanti
      await pool.query(
        `UPDATE broadcast_items
