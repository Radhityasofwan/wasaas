import { pool } from "./db";
// HAPUS IMPORT INI JIKA ADA: import { sendText } from "./wa";
...
export async function getBroadcastItems(jobId: number, tenantId: number, limit = 100, offset = 0) {
  // UPDATE: Select kolom reply_text
  const [rows] = await pool.query<any[]>(
...
// UPDATE: Parameter replyText ditambahkan
export async function markBroadcastReply(originalMessageId: string, fromNumber: string, replyText: string) {
  // FIX: Menghapus pengecekan 'to_number' agar lebih robust.
  // wa_message_id sudah unik. Ini mengatasi masalah jika nomor di DB '08xx' tapi reply dari '628xx'.
  const [res] = await pool.query<any>(
...
export async function handleBroadcastReply(tenantId: number, from: string, textBody: string, quotedMessageId: string | null) {
  ...
  // Pass textBody ke fungsi database
  const isReply = await markBroadcastReply(quotedMessageId, from, textBody);
  
  if (isReply) {
    console.log(`[Webhook] Broadcast reply detected from ${from}: ${textBody}`);
