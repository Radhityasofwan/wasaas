...
export async function handleBroadcastReply(tenantId: number, from: string, textBody: string, quotedMessageId: string | null) {
  ...
  if (isReply) {
    ...
    // Debugging: Pesan ini bukan balasan broadcast atau ID tidak ditemukan
    // console.log(`[DEBUG] Reply ignored. ID ${quotedMessageId} not found in broadcast_items.`);
  }
}
