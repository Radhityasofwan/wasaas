import { pool } from "./db";
// PENTING: Jangan import sendText di sini (Circular Dependency), pakai lazy import di bawah
...
export async function markBroadcastReply(originalMessageId: string, replyText: string) {
  // Update status jadi 'replied' jika wa_message_id cocok
  const [res] = await pool.query<any>(
...
export async function processBroadcastQueue() {
  const { sendText } = await import("./wa"); // Lazy Import (Wajib)
  
  // FIX: Gunakan perulangan (while) agar seluruh antrean diproses sampai habis dalam sekali trigger
  while (true) {
    const [rows] = await pool.query<any[]>(
      `SELECT
          bi.id AS item_id,
    ...
    // KELUAR DARI LOOP JIKA ANTREAN SUDAH HABIS / KOSONG
    if (!rows?.length) break;

    const row = rows[0];

    // Lock row untuk mencegah pengiriman ganda
    ...
    try {
      ...
      if (result.ok) {
        // SUCCESS: Simpan ID pesan agar bisa dilacak reply-nya
        await pool.query(
          `UPDATE broadcast_items
    ...
    // Cek apakah keseluruhan target pada Job ini sudah selesai
