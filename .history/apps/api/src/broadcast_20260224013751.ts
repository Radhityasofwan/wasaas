...
export async function processBroadcastQueue() {
  ...
  // FIX: LOOP AGAR MEMPROSES SELURUH TARGET DALAM SEKALI TRIGGER
  while (true) {
    const [rows] = await pool.query<any[]>(
      `SELECT
          bi.id AS item_id,
    ...
    // KELUAR DARI LOOP JIKA ANTREAN SUDAH HABIS
    if (!rows?.length) break;

    const row = rows[0];

    // Lock row (Mencegah terproses ganda)
    ...
    try {
      ...
      if (delay) await sleep(delay); // Pause sebelum kirim
    ...
    // Cek apakah job keseluruhan sudah selesai
