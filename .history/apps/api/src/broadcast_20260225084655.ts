  ...
  try {
    ...
    while (true) {
      ...
      try {
        ...
        // ========================================================
        let parsedText = String(row.text_body || "");

        // 1. Spintax: Mengacak kata {Halo|Hai|Permisi}
        parsedText = parsedText.replace(/\{([^{}]+)\}/g, (match, contents) => {
          const options = contents.split('|');
          return options[Math.floor(Math.random() * options.length)];
        });

        // 2. Variabel Waktu: {{salam}}
        const h = new Date().getHours();
        let salam = "Malam";
        if (h >= 3 && h < 11) salam = "Pagi";
        else if (h >= 11 && h < 15) salam = "Siang";
        else if (h >= 15 && h < 18) salam = "Sore";
        parsedText = parsedText.replace(/\{\{salam\}\}/ig, salam);

        // 3. Variabel Kontak Sederhana
        parsedText = parsedText.replace(/\{\{nomor\}\}/ig, row.to_number);
        parsedText = parsedText.replace(/\{\{nama\}\}/ig, "Kak"); // Fallback sementara jika bukan dari CRM Leads
