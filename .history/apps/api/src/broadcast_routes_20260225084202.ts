...
export async function createBroadcast(req: any, res: any) {
  const schema = z.object({
    ...
    text: z.string().max(4096).optional(), // Diubah menjadi opsional untuk dukungan media
    msgType: z.enum(['text', 'image', 'document', 'video']).default('text'), // Tambahan dukungan tipe pesan
    scheduledAt: z.string().optional() // Tambahan dukungan penjadwalan
  });
