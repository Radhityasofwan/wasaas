...
// FEATURE 2: Spintax & Dynamic Variable Engine
function parseMessageMagic(rawText: string | null, targetNumber: string): string {
  if (!rawText) return "";
  
  let processed = rawText;

  // 1. Spintax Parser: {Halo|Hai|Selamat Pagi} -> Pilih salah satu secara acak
  processed = processed.replace(/\{([^{}]+)\}/g, (match, contents) => {
    const options = contents.split('|');
    return options[Math.floor(Math.random() * options.length)];
  });

  // 2. Variable Parser: {{nama}} dan {{nomor}}
  // Karena saat ini database target hanya menyimpan nomor, kita gunakan nama kontak/leads
  // Jika nama tidak ada di masa depan, kita fallback menggunakan sapaan sopan "Kak"
  processed = processed.replace(/\{\{nama\}\}/ig, "Kak"); // TODO: Bisa ditarik dari tabel Contacts/Leads jika sudah digabungkan nanti.
...
export async function processFollowUpQueue() {
  ...
  try {
    ...
    for (const target of targets) {
      ...
      const finalCaptionOrText = parseMessageMagic(text_body, to_number);
