...
// ENGINE SPINTAX & VARIABLE (FIXED: URUTAN DIBALIK)
function parseMessageMagic(rawText: string | null, targetNumber: string, targetName: string | null): string {
  if (!rawText) return "";
  
  let processed = rawText;

  // 1. Variable Parser DULUAN
  // Mencegah kurung kurawal ganda termakan oleh regex Spintax
  ...
  // 2. Spintax Parser KEMUDIAN
  // {Sapaan A|Sapaan B} -> Pilih salah satu secara acak
...
export async function processFollowUpQueue() {
  ...
  try {
    ...
    // Gunakan LEFT JOIN untuk template, agar jika template hilang/bermasalah, 
    ...
    for (const target of targets) {
      ...
      // EKSEKUSI MAGIC PARSER (SPINTAX & VARIABEL)
