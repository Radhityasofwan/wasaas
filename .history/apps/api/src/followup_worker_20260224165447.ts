...
function parseMessageMagic(rawText: string | null, targetNumber: string, targetName: string | null): string {
  ...
  // A. Eksekusi Variabel Terlebih Dahulu!
  // Kita buat aman untuk format {{nama}} maupun format salah ketik {nama}
  ...
  // B. Eksekusi Spintax dengan Strict Regex
  // Regex ini memastikan hanya {kata|kata} yang memiliki TANDA PIPA (|) yang akan diproses.
  // Jika tidak ada tanda pipa, sistem TIDAK AKAN menganggapnya sebagai spintax.
  ...
  while ((match = spintaxRegex.exec(processed)) !== null) {
    ...
    spintaxRegex.lastIndex = 0; // Reset index setelah melakukan replace
  }
...
export async function processFollowUpQueue() {
  ...
  try {
    ...
    for (const target of targets) {
      ...
      // Validasi balasan sebelum menembak pesan
      if (trigger_condition === 'unreplied') {
        ...
        if (replies.length > 0) {
          console.log(`[FollowUp Worker] Target ${to_number} dibatalkan otomatis karena dibalas.`);
          await pool.query(`UPDATE followup_targets SET status = 'canceled', last_error = 'User already replied' WHERE id = ?`, [target_id]);
          continue;
        }
      }

      // Mengambil Push Name / Nama Kontak dari Database
      ...
      try {
        ...
        // EKSEKUSI PENGIRIMAN BERDASARKAN TIPE (MENDUKUNG SEMUA TIPE)
        if (message_type === 'text') {
          ...
          if (isNaN(lat) || isNaN(lng)) throw new Error("Format lokasi tidak valid (Harus berupa: lat,lng)");
          sendResult = await sendLocation({ tenantId: tenant_id, userId: 1, sessionKey: session_key, to: to_jid, latitude: lat, longitude: lng });
        } else {
          sendResult = { ok: false, error: `Tipe media tidak dikenali atau URL Media kosong (${message_type})` };
        }
