...
// SaaS routes
import { setWebhook, getWebhook } from "./webhook_routes"; // FIX: getWebhook ditambahkan
// Broadcast routes
...
// ... existing imports ...
import autoReplyRoutes from "./auto_reply_routes"; // <-- IMPORT BARU
...
// ===== DEV UI AUTH BYPASS =====
// Karena sistem login JWT belum jadi, frontend tidak mengirim x-api-key di awal.
// Middleware ini meng-inject auth tenant 1 secara otomatis HANYA di environment lokal 
// agar proses development UI (Dashboard) tidak terblokir.
const dashboardAuthMw = (req: any, res: any, next: any) => {
  ...
  // Jika ada x-api-key (production/postman), gunakan auth ketat asli
...
// ===== API KEYS (dashboard) =====
// 1. Rute API murni untuk di-fetch oleh React
app.use("/api-keys", dashboardAuthMw, apiKeysRoutes);

// 2. Penangkap Khusus Konflik Vite Proxy
// Mencegah munculnya "Cannot GET /-keys" atau "JSON mentah" saat pengguna me-refresh halaman UI
...
// FIX: Menambahkan GET route untuk webhook agar UI bisa fetching data saat ini
...
try {
  ...
  // ---> PERBAIKAN: Middleware auth ditambahkan di sini
...
app.use("/auto-reply", dashboardAuthMw, autoReplyRoutes); // <-- DAFTARKAN DISINI
