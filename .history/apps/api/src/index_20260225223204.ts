...
// FIX: Menambahkan pauseBroadcast dan resumeBroadcast
...
import { deleteApiKey } from "./api_keys";
...
// ============================================================================
// NEW: AUTHENTICATION / LOGIN ROUTE
// ============================================================================
app.post("/login", async (req: any, res: any) => {
  try {
    ...
    // (Karena pendaftaran di admin menggunakan teks biasa (plain text), kita cek biasa. Jika nanti pakai bcrypt, gunakan bcrypt.compare)
    if (user.password_hash !== password) {
      return res.status(401).json({ ok: false, error: "Email atau password salah." });
    }

    // 4. Generate API Key baru untuk sesi UI
    ...
    // 5. Revoke (cabut) key "UI Session" yang lama agar tidak menumpuk di database
    await pool.query(
      `UPDATE api_keys SET revoked_at=NOW() WHERE tenant_id=? AND user_id=? AND name='UI Session' AND revoked_at IS NULL`,
      [user.tenant_id, user.id]
    );

    // 6. Simpan key baru ke database
    await pool.query(
      `INSERT INTO api_keys(tenant_id, user_id, name, key_hash, created_at) VALUES(?, ?, 'UI Session', ?, NOW())`,
      [user.tenant_id, user.id, hashKey]
    );

    // 7. Kembalikan rawKey ke Frontend
...
// FIX: Tambahan routing untuk Pause dan Resume Broadcast
app.post("/broadcast/:id/pause", dashboardAuthMw, (req:any,res:any)=>pauseBroadcast(req,res));
app.post("/broadcast/:id/resume", dashboardAuthMw, (req:any,res:any)=>resumeBroadcast(req,res));

// Leads Route
app.get("/leads", dashboardAuthMw, (req:any,res:any)=>getLeads(req,res));
app.post("/leads/label", dashboardAuthMw, (req:any,res:any)=>setLeadLabel(req,res));
app.post("/leads/delete", dashboardAuthMw, (req:any,res:any)=>deleteLeads(req,res));
app.get("/leads/temp-rules", dashboardAuthMw, (req:any,res:any)=>getTempRules(req,res));
app.post("/leads/temp-rules", dashboardAuthMw, (req:any,res:any)=>saveTempRules(req,res));
...
// Endpoint khusus untuk hapus permanen
app.delete("/api/api-keys/:id/permanent", requireAuth, async (req: any, res: any) => {
  try {
    const id = parseInt(req.params.id);
    await deleteApiKey(req.auth.tenantId, id);
    res.json({ ok: true, message: "Kunci API berhasil dihapus secara permanen." });
  } catch (error: any) {
...
try {
  ...
  app.get("/admin/tenants", dashboardAuthMw, (req:any,res:any)=>getTenants(req,res));
  app.post("/admin/tenants", dashboardAuthMw, (req:any,res:any)=>createTenant(req,res));
  app.put("/admin/tenants/:id/limits", dashboardAuthMw, (req:any,res:any)=>updateTenantLimits(req,res));
} catch (e) {
