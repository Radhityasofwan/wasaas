...
export async function setLeadLabel(req: any, res: any) {
  ...
  try {
    ...
    // Jika nomor belum ada di CRM Leads, otomatis dibuat dengan status 'warm'. 
    // Jika sudah ada, cukup timpa/update tag labelnya.
    for (const num of parsed.data.targets) {
      const cleanNum = num.split('@')[0]; // Pastikan bersih dari akhiran JID @s.whatsapp.net
