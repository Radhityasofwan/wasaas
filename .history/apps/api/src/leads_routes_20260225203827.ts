...
async function ensureTempRulesSchema() {
  try {
    await pool.query(`SELECT hot_keywords FROM crm_temp_rules LIMIT 1`);
  } catch (e: any) {
...
export async function getTempRules(req: any, res: any) {
  try {
    ...
    if (rows.length === 0) {
      ...
      return res.json({ ok: true, data: { hot_keywords: "pesan,order,beli,harga,transfer", warm_sources: '["meta_ads","ig","tiktok","web"]', cold_days: 7 } });
    }
...
export async function saveTempRules(req: any, res: any) {
  ...
  try {
    await pool.query(
      `INSERT INTO crm_temp_rules (tenant_id, hot_keywords, warm_sources, cold_days) 
       VALUES (?, ?, ?, ?)
       ON DUPLICATE KEY UPDATE 
       hot_keywords = VALUES(hot_keywords),
       warm_sources = VALUES(warm_sources),
       cold_days = VALUES(cold_days)`,
      [req.auth.tenantId, parsed.data.hot_keywords, parsed.data.warm_sources, parsed.data.cold_days]
    );
