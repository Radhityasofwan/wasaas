...
export async function listConversations(req: any, res: any) {
   ...
   * 4) join wa_contacts (NEW) for display_name (pushName dari WA / Kontak)
   */
  const [rows] = await pool.query<any[]>(
    `SELECT
        c.id AS chat_id,
  ...
  const conversations = rows.map((r) => ({
    ...
    name: r.contact_name ?? null, // Parameter nama akan otomatis diserap oleh Frontend Inbox
...
// FIX: POST /ui/conversations/delete { sessionKey, peers }
...
export async function deleteConversations(req: any, res: any) {
  ...
  try {
    const normalizedPeers = peers.map(p => normalizeRemoteJid(p));
    ...
    // 1. Hapus dari daftar chat / kontak
    await pool.query(
      `DELETE FROM wa_chats WHERE tenant_id=? AND session_key=? AND remote_jid IN (${placeholders})`,
      queryParams
    );

    // 2. Hapus detail riwayat pesannya
    await pool.query(
      `DELETE FROM wa_messages WHERE tenant_id=? AND session_key=? AND remote_jid IN (${placeholders})`,
      queryParams
    );
