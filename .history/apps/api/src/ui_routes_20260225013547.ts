 ...
 * * V.5.0 Stable Fixes:
 * - Ekstraksi `pushName` dan `participant` secara aman dari field JSON mentah 
 * untuk mendukung tampilan nama pengirim di dalam Obrolan Grup.
 * - Validasi Zod yang ketat untuk mencegah SQL Injection & payload berbahaya.
 ...
 * seperti nama kontak dari anggota grup yang belum tersimpan di database.
 */
 ...
 * Menyertakan join ke tabel pesan untuk memunculkan cuplikan pesan terakhir,
 * serta mengekstrak 'pushName' dari pesan grup agar dapat ditampilkan.
 */
export async function listConversations(req: any, res: any) {
  ...
  try {
     ...
     * 2. JOIN dengan tabel `wa_messages` `m` untuk mendapatkan isi pesannya.
     * 3. LEFT JOIN dengan `wa_chats` `c` untuk mengambil `unread_count`.
     * 4. LEFT JOIN dengan `wa_contacts` `cont` untuk mengambil nama kontak yang tersimpan.
     */
    ...
    const conversations = rows.map((r) => {
      // Ekstraksi PushName dari JSON Mentah (Berguna untuk melihat siapa pengirim terakhir di Grup)
      let pushName = null;
      if (r.last_raw_json) {
      ...
      return {
        ...
        lastMessage: {
          ...
          time: r.last_time,
          pushName: pushName // Nama pengirim terakhir untuk pratinjau
        }
      };
    });
 ...
 * Menarik riwayat gelembung obrolan untuk suatu kontak/grup.
...
export async function listMessages(req: any, res: any) {
  ...
  try {
    ...
    const messages = rows.reverse().map((m) => {
      ...
      // FIX PENTING BUG GRUP: 
      // Menguraikan ID Pengirim asli (Participant) dan Nama Pengirim (PushName) 
      // khusus ketika pesan tersebut berasal dari dalam Grup.
      if (m.raw_json) {
      ...
      return {
        ...
        time: m.created_at,
        participant, // Diteruskan ke UI Inbox untuk Avatar/Warna Unik
        pushName     // Diteruskan ke UI Inbox untuk ditautkan di atas gelembung
      };
    });
 ...
 * Lebih ramah memori ketimbang menggunakan Socket.IO pada server monolitik.
 */
export async function streamSSE(req: any, res: any) {
  ...
  const interval = setInterval(async () => {
    try {
      ...
      // Menarik data pesan yang ID-nya lebih besar dari ID terakhir yang diketahui client
      const [rows] = await pool.query<any[]>(
