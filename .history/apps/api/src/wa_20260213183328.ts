...
import { pool } from "./db";
import { handleBroadcastReply } from "./broadcast"; // Import fitur reply tracking
...
console.log("WA_PARSER_VERSION=2 loaded");
...
export async function startSession(sessionKey: string, ctx: { tenantId: number; userId: number }) {
  ...
  sock.ev.on("connection.update", async (update) => {
    ...
    if (connection === "open") {
      ...
      await upsertSession(ctx.tenantId, ctx.userId, sessionKey, {
        status: "connected",
        wa_me_jid: me,
        phone_number: me ? me.split("@")[0] : null,
        last_seen_at: new Date(),
        last_error: null
      });

      console.log(`[${sessionKey}] connected as ${me ?? "unknown"}`);
    }
    ...
    if (connection === "close") {
      ...
      sessions.set(sessionKey, { ctx: { tenantId: ctx.tenantId, userId: ctx.userId }, sessionKey, sock: null, status: loggedOut ? "logged_out" : "disconnected" });

      await upsertSession(ctx.tenantId, ctx.userId, sessionKey, {
        status: loggedOut ? "logged_out" : "disconnected",
        last_error: loggedOut ? "loggedOut" : "disconnected",
        last_seen_at: new Date()
      });

      console.log(`[${sessionKey}] connection closed: ${loggedOut ? "loggedOut" : "disconnected"}`);
  ...
  sock.ev.on("messages.upsert", async (m) => {
    ...
    for (const msg of m.messages) {
      ...
      if (!parsed) {
        // Log debug jika pesan tidak bisa diparsing
        // console.log("DEBUG_RAW_MESSAGE=", JSON.stringify(msg.message, null, 2).slice(0, 2000));
        continue;
      }
      ...
      // --- CEK BALASAN BROADCAST (NEW) ---
      // Ambil ID pesan yang dibalas (quoted) jika ada
      ...
      if (quotedId) {
        // Cek apakah pesan yang dibalas adalah broadcast, jika ya trigger webhook
...
export async function stopSession(sessionKey: string) {
  try {
    ...
    if (sock) {
      ...
      try { sock.end?.(new Error("stopped")); } catch {}
    }
  } catch {}
