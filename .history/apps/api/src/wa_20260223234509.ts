...
export async function startSession(sessionKey: string, ctx: { tenantId: number; userId: number }) {
  ...
  sock.ev.on("messages.upsert", async (m) => {
    ...
    for (const msg of m.messages) {
      ...
      // --- NEW: LOGIKA BOT AUTO REPLY INTERNAL ---
      if (parsed.text && remoteJid && !remoteJid.includes('@g.us')) {
        try {
          // 1. Ambil rules aktif untuk tenant ini
          const [rules] = await pool.query<any[]>(
            `SELECT keyword, match_type, reply_text FROM auto_reply_rules WHERE tenant_id=? AND is_active=1`,
            [ctx.tenantId]
          );

          // 2. Cocokkan pesan masuk dengan kata kunci
          ...
          for (const rule of rules) {
            ...
            if (matchedReply) break; // Berhenti mencari jika sudah ada 1 rule yang cocok
          }

          // 3. Jika cocok, kirimkan balasannya otomatis!
          if (matchedReply) {
            console.log(`[${sessionKey}] Auto-replying to ${remoteJid}`);
            // Panggil sendText (fungsi yang sudah ada di wa.ts) untuk membalas
