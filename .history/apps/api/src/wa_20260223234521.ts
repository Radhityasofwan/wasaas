...
export async function startSession(sessionKey: string, ctx: { tenantId: number; userId: number }) {
  ...
  // ===== NEW: EVENT TRACKER UNTUK STATUS PESAN =====
  ...
  sock.ev.on("messages.upsert", async (m) => {
    ...
    for (const msg of m.messages) {
      ...
      if (parsed.text && remoteJid && !remoteJid.includes('@g.us')) {
        try {
          // Ambil rules aktif untuk tenant ini, dan filter HANYA untuk session_key ini ATAU yang bernilai NULL (Semua nomor)
          const [rules] = await pool.query<any[]>(
            `SELECT keyword, match_type, reply_text 
             FROM auto_reply_rules 
             WHERE tenant_id=? AND is_active=1 AND (session_key IS NULL OR session_key = ?)`,
            [ctx.tenantId, sessionKey]
          );
          ...
          // Kirim balasan
          if (matchedReply) {
            console.log(`[${sessionKey}] 🤖 Bot membalas ke ${remoteJid} karena match keyword!`);
            // Berikan jeda 1.5 detik agar terlihat lebih natural (tidak seperti robot instan)
            setTimeout(async () => {
              try { await sendText(sessionKey, remoteJid, matchedReply); } catch (e) { console.error("Auto Reply Send Failed:", e) }
            }, 1500);
          }
        } catch (err) {
