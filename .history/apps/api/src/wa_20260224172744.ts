...
export async function startSession(sessionKey: string, ctx: { tenantId: number; userId: number }) {
  ...
  sock.ev.on("messages.upsert", async (m) => {
    ...
    for (const msg of m.messages) {
      ...
      // FIX: SIMPAN PUSH NAME (NAMA KONTAK) KE DATABASE WA_CONTACTS
...
export async function sendText(sessionKey: string, to: string, text: string) {
  ...
  try {
    ...
    await insertMessage(tenantId, userId, {
      sessionKey,
export async function insertMessage(tenantId: number, userId: number, data: any) {
  // ... bagian yang sudah ada sebelumnya (yang meng-insert data ke wa_messages) ...
  await pool.query(`INSERT INTO wa_messages ...`);

  // 👇 TAMBAHKAN LOGIKA INI DI BAWAHNYA 👇
  // Jika pesan yang masuk adalah pesan dari klien (direction 'in'), tambahkan unread_count!
  if (data.direction === 'in') {
     await pool.query(
       `UPDATE wa_chats SET unread_count = unread_count + 1 
        WHERE tenant_id = ? AND session_key = ? AND remote_jid = ?`,
       [tenantId, data.sessionKey, data.remoteJid]
     );
  }
}
