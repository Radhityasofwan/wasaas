 ...
 * [NEW] Mendukung "Typing Indicator" (Sedang Mengetik...) dan Custom Delay.
 */
async function processAutoReply(tenantId: number, sessionKey: string, remoteJid: string, text: string) {
  try {
    let rules: any[] = [];
    
    // Fallback Aman jika database belum dimigrasi sempurna
    try {
      ...
      // Jika kolom delay_ms belum siap, gunakan fallback delay 2000ms
      [rules] = await pool.query<any[]>(
    ...
    if (matchedRule) {
      ...
      if (sock) {
         ...
         // Memaksa layar HP pelanggan memunculkan "sedang mengetik..."
      ...
      setTimeout(async () => {
        try { 
          if (sock) {
             ...
             // Matikan status mengetik sesaat sebelum pesan diluncurkan
             await sock.sendPresenceUpdate('paused', remoteJid);
          }
          await sendText(sessionKey, remoteJid, matchedRule.reply_text); 
        } catch (e) { 
