 ...
 * - [NEW] Typing Indicator ("Sedang Mengetik...") dengan Custom Delay.
 * ============================================================================
 */
...
console.log("WA_PARSER_VERSION=8.1 (Enterprise CRM Edition + Typing Indicator) loaded");
 ...
 * Mendukung "Typing Indicator" (Sedang Mengetik...) dan Custom Delay.
 */
async function processAutoReply(tenantId: number, sessionKey: string, remoteJid: string, text: string) {
  try {
    ...
    try {
      ...
      // Fallback jika belum migrasi kolom delay_ms
      [rules] = await pool.query<any[]>(
    ...
    if (matchedRule) {
      ...
      setTimeout(async () => {
        try { 
          if (sock) {
             // Matikan sinyal sedang mengetik
