 ...
 * * V.8.5 Ultimate CRM Edition (Stable Fix):
...
console.log("WA_PARSER_VERSION=8.5 (Enterprise CRM Edition + Stable Typing) loaded");
...
async function processAutoReply(tenantId: number, sessionKey: string, remoteJid: string, text: string) {
  try {
    ...
    for (const rule of rules) {
      ...
      if (matchedRule) {
        console.log(`[${sessionKey}] 🤖 Auto-Reply Matched: "${rule.keyword}" for JID: ${remoteJid}`);
        break; 
      }
    }
    ...
    if (matchedRule) {
      ...
      if (sock) {
         try {
           ...
           // Mengabaikan error wajar dari device @lid
      ...
      setTimeout(async () => {
        try { 
          ...
          // Mengeksekusi parser pesan agar auto reply mendukung variabel {{nama}}
  ...
  sock.ev.on("messages.upsert", async (m) => {
    ...
    for (const msg of m.messages) {
      try {
        ...
        // MEMANGGIL FUNGSI FOLLOW UP TRIGGER
