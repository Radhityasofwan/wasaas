 ...
 * * V.8.6 Ultimate CRM Edition (Multi-Keyword Fix):
 * - [FIX] Engine Auto Reply kini mendukung Multi-Keyword (pemisahan koma) secara cerdas.
 * - [FIX] Sinyal Composing (Sedang Mengetik) anti-crash pada target @lid.
 * - [FIX] Restored processFollowUpRepliedTrigger yang sempat terpotong.
 ...
 * - Graceful Error Handling & Strict Typing ekstensif (> 900 Lines).
 * ============================================================================
 */
...
console.log("WA_PARSER_VERSION=8.6 (Enterprise CRM Edition + Multi Keyword Fix) loaded");
...
// ============================================================================
// 3. PARSER ENGINE (SPINTAX, VARIABLES & PROTOS)
// ============================================================================

/**
 * Menghasilkan sapaan berdasarkan waktu lokal (WIB/GMT+7)
 */
...
/**
 * Engine Spintax dan Variabel Dinamis (Digunakan di Auto Reply & Follow Up)
 */
export function parseMessageMagic(rawText: string | null, targetNumber: string, targetName: string | null): string {
  ...
  // 1. Eksekusi Variabel Terlebih Dahulu (Nama & Nomor)
  ...
  processed = processed.replace(/\{nama\}/ig, finalName); // Fallback antisipasi typo

  processed = processed.replace(/\{\{nomor\}\}/ig, targetNumber);
  processed = processed.replace(/\{nomor\}/ig, targetNumber);

  // 2. Eksekusi Variabel Salam Waktu
  const salamWaktu = getDynamicGreeting();
  processed = processed.replace(/\{\{salam\}\}/ig, salamWaktu);
  processed = processed.replace(/\{salam\}/ig, salamWaktu);

  // 3. Eksekusi Spintax dengan Strict Regex ({Halo|Hai})
  ...
  while ((match = spintaxRegex.exec(processed)) !== null) {
    ...
    spintaxRegex.lastIndex = 0; // Reset index setelah replace
  }
...
/**
 * Membuka bungkus (unwrap) tipe pesan kompleks seperti ephemeral, view once, atau edited.
 */
...
/**
 * Mengubah objek protobuf Baileys menjadi struktur JSON yang mudah dimengerti.
 */
function parseContent(webMsg: proto.IWebMessageInfo): ParsedContent | null {
  ...
  // Teks Extended (Forwarded / Formatting)
...
/**
 * Menyimpan atau memperbarui Status Sesi Perangkat ke Database.
 */
...
/**
 * Membuat utas obrolan (Chat Thread) baru jika belum ada.
 */
...
/**
 * Menyimpan Pesan Masuk / Keluar ke Database Utama secara Presisi.
 */
 ...
 * Mendukung "Typing Indicator" (Sedang Mengetik...) yang Tahan Banting (Anti-Crash)
 * serta Dynamic Variables Parser ({{nama}}).
 * [FIX] Mendukung MULTI-KEYWORD (dipisahkan koma) secara cerdas.
 */
async function processAutoReply(tenantId: number, sessionKey: string, remoteJid: string, text: string) {
  try {
    let rules: any[] = [];
    
    // Fallback Aman jika database belum dimigrasi (kolom delay_ms belum ada)
    try {
    ...
    // Pencocokan Logika Bot (Multi-Keyword Berbasis Koma)
    for (const rule of rules) {
      // Pecah keyword berdasarkan koma, bersihkan spasi ekstra, lalu iterasi
      ...
      // Cek setiap variasi keyword di dalam satu rule
      for (const kw of keywordsArray) {
        ...
        if (isMatch) {
          matchedWord = kw; // Menyimpan kata spesifik yang berhasil memicu bot
          break; 
        }
      }
      ...
      if (isMatch) {
        ...
        break; // Hentikan pencarian rule lain jika sudah ketemu 1 yang cocok
      }
    }

    // Jika Ada Rule yang Cocok, Eksekusi Pengiriman
    if (matchedRule) {
      const delay = matchedRule.delay_ms || 2000;
      const sock = getSessionSock(sessionKey);
      
      // TRIGGER TYPING INDICATOR (Dengan proteksi penuh)
      if (sock) {
         try {
           await sock.sendPresenceUpdate('composing', remoteJid);
         } catch (err) {
           // Mengabaikan error wajar dari device companion @lid
           console.warn(`[${sessionKey}] Sinyal composing diabaikan untuk target khusus: ${remoteJid}`);
         }
      }
      
      // Delay organik (Anti-Banned WA) sebelum mengirim pesan
      setTimeout(async () => {
        try { 
          if (sock) {
             try { 
          ...
          // Mengambil nama kontak dari CRM untuk Variables Engine
          ...
          // Mengeksekusi parser pesan agar auto reply mendukung variabel {{nama}} dan Spintax
  ...
  sock.ev.on("messages.upsert", async (m) => {
    ...
    for (const msg of m.messages) {
      try {
        ...
        // --------------------------------------------------------------------
        // [SMART TEMPERATURE ENGINE] Penentuan Suhu Otomatis
        // --------------------------------------------------------------------
        let autoStatus = 'warm'; // Default: Hangat
        
        // Jika pelanggan membalas pesan penawaran kita (Broadcast/Follow Up), 
        // atau bertanya secara spesifik, naikkan suhunya jadi HOT!
        if (isBroadcastReply || isFollowUpReply || txtLower.includes('pesan') || txtLower.includes('order') || txtLower.includes('beli') || txtLower.includes('harga')) {
           autoStatus = 'hot';
        }

        // Simpan ke Master Data Leads.
        // Logika CASE WHEN: Jika status saat ini sudah 'converted' atau 'dead', jangan ditimpa oleh sistem otomatis.
        await pool.query(
          ...
          [ctx.tenantId, cleanNumber, pushName, leadSource, autoStatus, pushName, autoStatus]
        );
