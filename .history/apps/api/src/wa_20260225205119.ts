 ...
 * * V.8.7 Ultimate CRM Edition (Flexible Engine):
 * - [NEW] Smart Engine sangat fleksibel (Multi-Choice: Source & Keywords).
 * - [NEW] Melacak Nama Campaign Meta Ads (Ad Reply) secara otomatis.
...
console.log("WA_PARSER_VERSION=8.7 (Enterprise CRM Edition + Meta Ads Tracking) loaded");
  ...
  sock.ev.on("messages.upsert", async (m) => {
    ...
    for (const msg of m.messages) {
      try {
        ...
        // Cek Quoted Message (Balasan Broadcast / Follow Up)
        const quotedId = 
        ...
        // Tentukan Sumber Trafik & Nama Campaign Iklan
        let leadSource = 'organic';
        
        if (adReply) {
           // Ekstraksi nama judul iklan (title) atau deksripsi (body) yang disisipkan oleh Meta
           const adName = adReply.title || adReply.body || 'Iklan';
           // Format: meta_ads|Nama_Campaign
        ...
        if (tempRules) {
           // 1. EVALUASI WARM (Berdasarkan Sumber Trafik DAN Kata Kunci)
           let isWarm = false;
           try {
             // Cek dari sumber trafik (cek kata dasarnya sebelum tanda pipa '|')
           ...
           // 2. EVALUASI HOT (Berdasarkan Kata Kunci DAN Sumber Trafik)
           let isHot = false;
           try {
             // Cek dari sumber trafik khusus Hot
             const baseSource = leadSource.split('|')[0];
             const hotSourcesArray = JSON.parse(tempRules.hot_sources || "[]");
             if (hotSourcesArray.includes(baseSource)) isHot = true;

             // Cek dari kata kunci Hot
           ...
           // Hierarki Penentuan: Hot mengalahkan Warm, Warm mengalahkan Cold
           ...
           // Legacy Fallback Jika CS belum mengatur rule
           autoStatus = 'cold';
           if (adReply || leadSource !== 'organic') autoStatus = 'warm';
           if (isBroadcastReply || isFollowUpReply || txtLower.includes('pesan') || txtLower.includes('order')) autoStatus = 'hot';
        }

        // Simpan ke Master Data Leads (Menghargai status Converted/Dead agar tidak tertimpa)
        await pool.query(
          `INSERT INTO crm_leads (tenant_id, phone_number, name, source, status, last_interacted_at, created_at)
           VALUES (?, ?, ?, ?, ?, NOW(), NOW())
           ON DUPLICATE KEY UPDATE 
           name = COALESCE(?, name),
           source = CASE WHEN source = 'manual' OR source = 'organic' THEN ? ELSE source END,
           status = CASE 
