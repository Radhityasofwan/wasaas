...
async function processAutoReply(tenantId: number, sessionKey: string, remoteJid: string, text: string) {
  try {
    ...
    if (matchedRule) {
      ...
      // FIX 408 TIMEOUT: WhatsApp Server menolak presenceUpdate ("composing") ke alamat LID (@lid) 
      // yang mengakibatkan socket hang dan terputus dengan Code 408. Kita harus mem-bypass ini.
      ...
      setTimeout(async () => {
        try { 
          // FIX: Ambil ulang sock karena kemungkinan koneksi terputus dan direstart saat delay berjalan
