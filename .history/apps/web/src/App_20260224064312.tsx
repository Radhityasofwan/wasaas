...
// --- MOCK COMPONENTS FOR CANVAS PREVIEW ---
// These replace the missing local imports so the Canvas can render the UI.
const MockPage = ({ title, description }: { title: string, description?: string }) => (
  <div className="w-full h-full min-h-[60vh] flex flex-col items-center justify-center bg-white rounded-3xl border border-slate-200 shadow-sm p-8 text-center">
    <div className="w-16 h-16 bg-blue-50 text-blue-500 rounded-2xl flex items-center justify-center mb-4">
      <Layers size={32} />
    </div>
    <h2 className="text-slate-800 font-bold text-2xl mb-2">{title}</h2>
    <p className="text-slate-500 text-sm max-w-md">
      {description || "This is a mock component rendered in the Canvas preview to demonstrate the layout structure."}
    </p>
  </div>
);

const Login = () => <MockPage title="Login / Authentication" description="The user would enter their API key or credentials here." />;
const Sessions = () => <MockPage title="WhatsApp Sessions" />;
const Inbox = () => <MockPage title="Unified Inbox" />;
const Webhooks = () => <MockPage title="Webhook Management" />;
const Broadcast = () => <MockPage title="Broadcast Campaigns" />;
const Leads = () => <MockPage title="Leads & Contacts" />;
const ApiKeys = () => <MockPage title="API Key Management" />;
const AutoReply = () => <MockPage title="Auto Reply Rules" />;
const Admin = () => <MockPage title="Admin Dashboard" />;
const Docs = () => <MockPage title="API Documentation" />;
const Templates = () => <MockPage title="Message Templates" />;
const AutoFollowUp = () => <MockPage title="Auto Follow-up Sequences" />;

// --- MOCK UTILS ---
const getApiKey = () => "mock-api-key-for-preview";
const clearApiKey = () => console.log("API Key cleared (simulated logout)");
// ------------------------------------------
