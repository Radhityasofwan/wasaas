...
function Shell({ children }: { children: React.ReactNode }) {
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full bg-[#f8fafd] safe-area-pb">
      {/* Brand Logo - Menggunakan custom logo URL */}
  ...
  return (
    <div className="flex h-screen w-full bg-white overflow-hidden text-slate-800 font-sans">
      ...
      <div className="flex-1 flex flex-col h-full min-w-0 bg-white">
        ...
        <header className="md:hidden h-14 bg-white border-b border-slate-100 flex items-center justify-between px-4 shrink-0 safe-area-pt">
          <div className="flex items-center gap-3">
            <button 
              onClick={() => setIsMobileMenuOpen(true)}
              className="p-1.5 -ml-1.5 text-slate-600 hover:bg-slate-100 rounded-full active:bg-slate-200 transition-colors"
            >
              <Menu size={22} />
            </button>
