...
export default function Admin() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      // FIX: Hapus prefix "admin/" agar sinkron dengan admin_routes.ts
      const [tRes, pRes] = await Promise.all([
        apiFetch<any>("tenants").catch(() => ({ data: [] })),
        apiFetch<any>("plans").catch(() => ({ data: [], plans: [] }))
      ]);
  ...
  const handleSaveLimits = async (e: React.FormEvent) => {
    ...
    try {
      // FIX: Hapus prefix "admin/" agar sinkron dengan admin_routes.ts
      await apiFetch(`tenants/${editingTenant.tenant_id}/limits`, {
        method: "PUT",
        body: JSON.stringify({
    ...
    <div className="space-y-12 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col xl:flex-row justify-between gap-8">
        <div>
          ...
          <div className="mt-8 flex gap-4">
            ...
            <button 
              onClick={() => nav('/billing')}
              className="px-8 py-4 bg-white text-blue-600 border-2 border-blue-100 rounded-[1.5rem] font-black text-[11px] uppercase tracking-widest hover:bg-blue-50 hover:scale-105 transition-all flex items-center gap-2"
            >
      ...
      {addModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-8 bg-slate-900/60 backdrop-blur-2xl animate-in fade-in duration-300">
          <div className="w-full max-w-2xl bg-white/95 backdrop-blur-3xl rounded-[3rem] p-10 sm:p-12 shadow-2xl border border-white relative overflow-hidden">
            ...
            <form onSubmit={handleCreateTenant} className="space-y-6 relative z-10">
              <div className="space-y-3">
                ...
                <input 
                  ...
                  className="w-full px-6 py-4 rounded-2xl bg-slate-50 border border-slate-200 font-bold text-slate-800 outline-none focus:bg-white focus:ring-2 focus:ring-blue-100"
                />
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="space-y-3">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Email Pemilik</label>
                  <input 
              ...
              <div className="bg-slate-50 p-6 rounded-[2rem] border border-slate-100 space-y-4">
                <div className="flex justify-between items-center">
                   ...
                   {plans.length === 0 && (
                     <button type="button" onClick={() => nav('/billing')} className="text-[10px] font-bold text-blue-500 hover:underline">Buat Paket Dulu →</button>
                   )}
                </div>
                <select 
                  required
                ...
                <p className="text-[10px] text-slate-500 font-medium">Jika kosong, tutup modal ini dan klik tombol "Kelola Paket (Billing)"</p>
              </div>
