...
export default function AutoFollowUp() {
  ...
  // UseRef untuk mencegah interval bertabrakan dengan state stale
  ...
  async function loadData(showLoading = false) {
    try {
      ...
      if (sessRes.sessions?.length > 0 && !formData.session_key) {
  ...
  async function loadTargets(campaignId: number) {
    try {
      ...
      // silent fetch error for background polling
    }
  }

  // REALTIME POLLING ENGINE (Setiap 5 Detik Update Background)
  useEffect(() => {
    loadData(true); // Load perdana dengan loading screen

    const pollTimer = setInterval(() => {
      loadData(false); // Silent background reload untuk data campaigns
      if (selectedCampRef.current?.id) {
        loadTargets(selectedCampRef.current.id); // Silent reload untuk target
      }
    }, 5000);
  ...
  return (
    <div className="flex h-full max-h-[85vh] bg-transparent overflow-hidden rounded-[2.5rem] relative">
      ...
      <div className="w-full md:w-[350px] lg:w-[400px] flex flex-col border-r border-white/20 bg-white/30 backdrop-blur-3xl shrink-0 relative z-10">
        ...
        {/* Sidebar Header dengan Live Clock */}
