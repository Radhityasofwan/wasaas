...
export default function AutoReply() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      // Fetch Rules & Sessions secara paralel
      const [rulesRes, sessionsRes] = await Promise.all([
        apiFetch<any>("auto-reply"),
        apiFetch<any>("sessions") // Asumsi rute ini ada di sessions_routes.ts
      ]);
                  ...
                  <select value={selectedSession} onChange={(e) => setSelectedSession(e.target.value)} className="w-full px-8 py-5 rounded-2xl bg-white/60 border border-white font-bold text-slate-800 outline-none focus:bg-white appearance-none cursor-pointer">
                     ...
                     {activeSessions.map(session => (
                       <option key={session.session_key} value={session.session_key}>
                         {session.label || session.phone_number || session.session_key}
                       </option>
                     ))}
                  </select>
               </div>
