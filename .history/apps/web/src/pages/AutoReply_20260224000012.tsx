...
export default function AutoReply() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (!cleanKeyword || !cleanReplyText) return;
    ...
    try {
      await apiFetch("auto-reply", { 
        method: "POST", 
        body: JSON.stringify({ 
          session_key: cleanSession || null, // Jika kosong, pastikan menjadi null
  ...
  // Helper untuk menampilkan nama sesi yang mudah dibaca di tabel
  const getSessionDisplay = (sessionKey: string | null) => {
    if (!sessionKey) return "🌐 SEMUA NOMOR";
