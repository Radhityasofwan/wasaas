...
import { Bot, Plus, X, Edit3, Trash2, Smartphone, MessageSquare, Power, Clock } from "lucide-react";
...
export default function AutoReply() {
  ...
  const [delayMs, setDelayMs] = useState("2000"); // Default 2 Detik (2000ms)
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-500 font-bold uppercase text-[10px] tracking-widest mt-1">Balas Pesan Otomatis Berbasis Kata Kunci</p>
          </div>
        </div>
        ...
        <div className="bg-white/40 backdrop-blur-3xl border border-white rounded-[2.5rem] p-8 shadow-2xl shadow-indigo-500/5 animate-in slide-in-from-top-4 duration-500 relative overflow-hidden">
          ...
          <form onSubmit={handleSave} className="space-y-8 relative z-10">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
               ...
               <div className="space-y-3">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2 block">Tipe Pencocokan</label>
                  <div className="relative">
                    <select value={matchType} onChange={(e) => setMatchType(e.target.value as any)} className="w-full px-5 py-4 rounded-2xl bg-white/80 border border-white font-bold text-slate-700 outline-none focus:bg-white focus:ring-[4px] focus:ring-indigo-500/10 appearance-none cursor-pointer shadow-sm">
                       ...
                       <option value="startswith">🚀 Diawali Dengan (Starts)</option>
                    </select>
                    <div className="absolute right-5 top-1/2 -translate-y-1/2 pointer-events-none text-slate-400 text-xs">▼</div>
                  </div>
               </div>
               ...
               <div className="space-y-3">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2 block">Kata Kunci (Keyword)</label>
                  <input 
               ...
               <div className="space-y-3">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2 block">Jeda Mengetik (Ms)</label>
                  <div className="relative">
                    <input 
                  ...
                  <p className="text-[9px] text-slate-500 font-medium ml-2 mt-1 italic">1000 MS = 1 Detik.<br/>Memunculkan "sedang mengetik..."</p>
               </div>
            ...
            <div className="space-y-3">
              ...
              <textarea 
                ...
                placeholder="Ketik pesan balasan otomatis yang akan dikirim bot..." required 
              />
            </div>
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="bg-white/10 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-white/40">
                    <th className="px-8 py-6">Kondisi & Keyword</th>
                  ...
                  {rules.map(rule => (
                    <tr key={rule.id} className={`group transition-all ${rule.is_active ? 'hover:bg-white/40' : 'bg-slate-50/50 opacity-70 grayscale-[30%]'}`}>
                      <td className="px-8 py-6">
                         ...
                         <div className="font-black text-slate-800 text-[17px] tracking-tight ml-1 flex items-center gap-2">
                      ...
                      <td className="px-8 py-6">
                         <div className="max-w-xs p-4 rounded-2xl bg-white/60 border border-white text-xs font-medium text-slate-600 leading-relaxed shadow-sm line-clamp-2 relative">
