...
type JobRow = {
  ...
  sent_count: number;
  failed_count: number;
  text_body?: string; // Optional jika backend mengirim ini
  created_at: string;
  updated_at: string;
};
...
export default function Broadcast() {
  ...
  async function deleteJob(id: number) {
    ...
    try {
      ...
      // Hapus dari state lokal agar cepat
      setJobs(prev => prev.filter(j => j.id !== id));
      // Jika sedang melihat detail job ini, tutup modal
  ...
  async function openDetail(job: JobRow) {
    ...
    try {
      // Ambil detail items (nomor dan status)
      const r = await apiFetch<any>(`/broadcast/${job.id}/items?limit=500`);
      setJobItems(r.data || []);
    } catch (e: any) {
      console.error(e);
    } finally {
                ...
                <tr key={j.id} style={{ borderTop: "1px solid #1f2c33" }}>
                  ...
                  <td style={td}>
                    <div style={{display:'flex', gap: 6}}>
                      {/* Tombol View Detail */}
                      <button style={btnSmall} onClick={() => openDetail(j)} title="View Details">
                        👁️
                      </button>
                      
                      {/* Tombol Cancel (hanya jika running/queued) */}
                      {(j.status === "queued" || j.status === "running") && (
                        <button style={btnSmallWarning} onClick={() => cancel(j.id)} title="Stop Broadcast">
                          ⏹️
                        </button>
                      )}

                      {/* Tombol Delete */}
                      <button style={btnSmallDanger} onClick={() => deleteJob(j.id)} title="Delete History">
                        🗑️
                      </button>
                    </div>
                  </td>
        ...
        <div style={modalOverlay}>
          <div style={modalContent}>
            ...
            <div style={{marginBottom: 14, padding: 10, background: '#0b141a', borderRadius: 8, fontSize: 13}}>
               ...
               <div style={{whiteSpace: 'pre-wrap'}}>{viewJob.text_body || "(Text not available in list view)"}</div>
            </div>
                      ...
                      <tr key={item.id} style={{borderBottom:'1px solid #1f2c33'}}>
                        ...
                        <td style={{padding: 8}}>
                          {item.reply_status === 'replied' ? (
                            <span style={{color: '#53bdeb', fontWeight:'bold'}}>↩️ Replied</span>
                          ) : '-'}
                        </td>
