            ...
            <tbody>
              {filteredJobs.map(j => (
                <tr key={j.id} style={{ borderTop: "1px solid #1f2c33" }}>
                  ...
                  {/* Diperbaiki: Menggabungkan style dengan spread operator */}
                  <td style={{ ...td, fontSize: 11, opacity: 0.8 }}>{fmtDate(j.updated_at)}</td>
                  <td style={td}>
              ...
              {/* Diperbaiki: Menggabungkan style dengan spread operator */}
              {!filteredJobs.length && <tr><td colSpan={5} style={{ ...td, textAlign: 'center', opacity: 0.5 }}>Tidak ada data yang cocok</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
