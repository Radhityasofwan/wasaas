...
import { 
  ...
  Image as ImageIcon, FileText, Type, Clock, CheckCircle2 
} from "lucide-react";
...
export default function Broadcast() {
  ...
  async function loadSessions() {
    ...
    } catch(e) { console.error("Gagal load session", e); }
  }
  ...
  useEffect(() => {
    loadSessions(); 
    loadJobs();
    const t = setInterval(loadJobs, 3000); // Polling lebih cepat untuk animasi progress
  ...
  async function create() {
    ...
    // Saat ini backend murni menerima JSON text. Jika Media dipilih, kita beri notifikasi
    if (msgType !== 'text') {
  ...
  async function cancelJob(id: number) {
    ...
    try { await apiFetch(`broadcast/${id}/cancel`, { method: "POST" }); loadJobs(); } 
      ...
      <div className="bg-white/40 backdrop-blur-3xl border border-white rounded-[2.5rem] p-8 shadow-sm">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-400 font-medium ml-2 mt-2 leading-relaxed">
                ...
                <span className="text-blue-500 font-bold">Tips:</span> Fitur <i>Dynamic Variables</i> seperti <code>628xx|Budi</code> akan segera hadir!
              </p>
            </div>
          </div>
          ...
          <div className="space-y-6 flex flex-col h-full">
             ...
             {msgType !== 'text' && (
                <div className="w-full border-2 border-dashed border-slate-300 rounded-[2rem] p-6 text-center hover:bg-slate-50 transition-colors cursor-pointer bg-white">
                   <div className="w-12 h-12 rounded-full bg-slate-100 flex items-center justify-center mx-auto mb-3 text-slate-400">
                      {msgType === 'image' ? <ImageIcon size={20} /> : <FileText size={20} />}
                   </div>
                   <p className="text-xs font-bold text-slate-600">Klik untuk unggah {msgType === 'image' ? 'Gambar' : 'Dokumen'}</p>
                   <p className="text-[10px] text-slate-400 mt-1">Maks. ukuran 10MB</p>
                </div>
             )}

             <div className="flex-1 flex flex-col">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2 mb-3 block">Konten Pesan {msgType !== 'text' && '(Caption)'}</label>
                <textarea value={text} onChange={(e) => setText(e.target.value)} className="flex-1 w-full px-6 py-4 rounded-[2rem] bg-white border border-slate-200 font-medium text-slate-700 outline-none focus:ring-[6px] focus:ring-blue-500/10 transition-all resize-none text-sm leading-relaxed min-h-[140px]" placeholder="Halo, ini adalah pesan broadcast..." />
             </div>
