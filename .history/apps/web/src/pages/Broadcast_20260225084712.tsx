...
import { 
  ...
  Image as ImageIcon, FileText, Type, Clock, CheckCircle2, Layers 
} from "lucide-react";
...
type JobRow = {
  id: number; session_key: string; status: string; delay_ms: number;
  total_targets: number; sent_count: number; failed_count: number; updated_at: string;
};
type BroadcastItem = {
  id: number; to_number: string; status: string; reply_status: string; reply_text: string | null;
};
...
export default function Broadcast() {
  ...
  async function togglePause(id: number, currentStatus: string) {
    ...
    try { 
      ...
      loadJobsOnly(); 
    } catch (e: any) { setErr(`Fitur ${action} error.`); }
  }
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700 pb-20">
      ...
      {err && (
        <div className="bg-rose-50/80 border border-rose-100 text-rose-600 px-6 py-4 rounded-2xl text-[12px] font-bold flex items-center gap-3 backdrop-blur-md">
      ...
      {info && (
        <div className="bg-emerald-50/80 border border-emerald-100 text-emerald-600 px-6 py-4 rounded-2xl text-[12px] font-bold flex items-center gap-3 backdrop-blur-md">
               ...
               <select 
                 ...
                 onChange={(e) => {
                    ...
                    if(tpl) {
                       ...
                       // Opsional: Handle tpl.media_url jika Anda sudah mengaktifkan state media
                    }
                 }}
               >
