...
async function apiFetch<T>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  // FIX: Sinkronisasi Path Proxy (Menambahkan /api/)
...
export default function Broadcast() {
  ...
  // LIVE PARSER PREVIEW ENGINE (Disesuaikan dengan Backend Worker)
  const parsedPreview = useMemo(() => {
    let txt = text || "";
    if (!txt.trim()) return "";

    // 1. Live Variable Parser (Support {{nama}} dan {nama})
    ...
    // 2. Live Spintax Parser (Strict Mode: Hanya parse jika ada tanda pipa '|')
    ...
    while ((match = spintaxRegex.exec(processed)) !== null) {
      ...
      spintaxRegex.lastIndex = 0; // Reset index
    }
  ...
  async function loadInitialData() {
    try {
      ...
      // Filter template untuk general atau broadcast
      const validTpls = (tRes.data || []).filter((t: any) => t.category !== 'follow_up');
