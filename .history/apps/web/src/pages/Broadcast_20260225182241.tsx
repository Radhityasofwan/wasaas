...
import { 
  ...
  Image as ImageIcon, FileText, Type, Clock, CheckCircle2, Layers, RefreshCw, CheckSquare, XCircle
} from "lucide-react";
...
export default function Broadcast() {
  ...
  async function executeBulkDeleteJobs() {
    if (!confirm(`Hapus permanen ${selectedJobs.length} riwayat broadcast beserta antrean targetnya?`)) return;
    try {
      // Loop frontend delete untuk bulk delete tanpa mengubah backend routes
      ...
      alert("Gagal menghapus beberapa riwayat: " + e.message);
    }
  }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-6 flex flex-col h-full">
             ...
             <div className="flex-1 flex flex-col">
                ...
                {/* TOMBOL INSERT CEPAT */}
                <div className="mt-3 bg-blue-50/50 border border-blue-100 rounded-2xl p-4">
                  <div className="flex flex-wrap gap-2">
                    <button onClick={() => setText(text + '{{nama}}')} className="px-3 py-1.5 bg-white border border-blue-200 text-blue-600 rounded-lg text-xs font-bold shadow-sm hover:bg-blue-600 hover:text-white transition-colors">
                      + {"{{nama}}"}
                    </button>
                    <button onClick={() => setText(text + '{{nomor}}')} className="px-3 py-1.5 bg-white border border-blue-200 text-blue-600 rounded-lg text-xs font-bold shadow-sm hover:bg-blue-600 hover:text-white transition-colors">
                      + {"{{nomor}}"}
                    </button>
                    <button onClick={() => setText(text + 'Selamat {{salam}}')} className="px-3 py-1.5 bg-white border border-blue-200 text-blue-600 rounded-lg text-xs font-bold shadow-sm hover:bg-blue-600 hover:text-white transition-colors">
                      + {"{{salam}}"}
                    </button>
                    <button onClick={() => setText(text + '{Halo|Hai|Permisi}')} className="px-3 py-1.5 bg-white border border-blue-200 text-blue-600 rounded-lg text-xs font-bold shadow-sm hover:bg-blue-600 hover:text-white transition-colors">
                      Spintax {"{A|B}"}
                    </button>
                  </div>
                </div>
             ...
             <div className="mt-4 border border-slate-200 rounded-[1.5rem] bg-slate-50 overflow-hidden shadow-inner">
                <div className="px-4 py-3 border-b border-slate-200 bg-slate-100/50 flex justify-between items-center">
                  ...
                  <button type="button" onClick={() => setPreviewTrigger(p => p + 1)} className="text-[9px] font-bold bg-white border border-slate-200 text-slate-600 px-3 py-1.5 rounded-lg flex items-center gap-1.5 hover:text-blue-600 hover:border-blue-200 transition-colors shadow-sm cursor-pointer">
        ...
        <div className="px-8 py-6 border-b border-white/40 flex flex-col sm:flex-row justify-between items-center gap-4">
           <div className="flex items-center gap-4">
             ...
             <button 
               ...
               className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${isSelectionMode ? 'bg-indigo-600 text-white shadow-md' : 'bg-white border border-slate-200 text-slate-500 hover:bg-slate-50 hover:text-indigo-600'}`}
             >
               {isSelectionMode ? <XCircle size={14}/> : <CheckSquare size={14}/>} 
               {isSelectionMode ? 'Batal' : 'Mode Pilih'}
             </button>
           </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="text-left bg-white/10 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-white/20">
                {isSelectionMode && <th className="px-6 py-5 w-10 text-center">Pilih</th>}
            ...
            <tbody className="divide-y divide-white/20">
              {filteredJobs.length === 0 && (
                <tr><td colSpan={isSelectionMode ? 5 : 4} className="text-center py-12 text-slate-400 font-bold text-sm">Belum ada riwayat broadcast.</td></tr>
              )}
              {filteredJobs.map(j => {
                  ...
                  {isSelectionMode && (
                    <td className="px-6 py-5 align-middle text-center">
                      <div 
                        ...
                        className={`w-6 h-6 mx-auto rounded-md border-2 cursor-pointer flex items-center justify-center transition-all ${selectedJobs.includes(j.id) ? 'bg-indigo-600 border-indigo-600 text-white' : 'bg-white border-slate-300'}`}
                      >
        ...
        {isSelectionMode && selectedJobs.length > 0 && (
          <div className="absolute bottom-6 left-1/2 -translate-x-1/2 bg-slate-900 rounded-[2rem] p-4 shadow-2xl border border-slate-700 flex items-center gap-6 animate-in slide-in-from-bottom-10 z-50">
             <span className="text-[11px] font-black text-white px-4 py-2 bg-slate-800 rounded-xl border border-slate-700">
               {selectedJobs.length} Riwayat Terpilih
             </span>
             <div className="flex gap-2">
                <button onClick={executeBulkDeleteJobs} className="flex items-center gap-2 px-5 py-2 bg-rose-500 hover:bg-rose-600 rounded-xl text-white font-black text-[10px] uppercase tracking-widest transition-all shadow-lg">
