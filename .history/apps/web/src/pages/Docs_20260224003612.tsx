...
export default function Docs() {
  ...
  const tabs: { id: Tab; label: string; icon: string }[] = [
    { id: "auth", label: "Otentikasi & Limit", icon: "🔑" },
          ...
          {activeTab === "auth" && (
            <div className="space-y-12 animate-in slide-in-from-right-8 duration-500">
              <section className="space-y-4">
                ...
                <div className="bg-slate-900 rounded-[2.5rem] p-8 shadow-2xl relative overflow-hidden group mt-6">
...
<span className="text-pink-400">curl</span> --request GET \
  ...
  --header <span className="text-emerald-400">'Content-Type: application/json'</span>
              ...
              <section className="space-y-6">
                <h2 className="text-2xl font-black text-slate-800 tracking-tight">Rate Limits & Error Codes</h2>
                <p className="text-sm text-slate-500 font-medium leading-relaxed max-w-2xl">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-6 rounded-[2rem] bg-rose-50/50 border border-rose-100">
                    <span className="text-rose-600 font-black text-sm uppercase tracking-widest">HTTP 401 / 403</span>
                    <p className="text-xs text-rose-500/80 font-bold mt-2">API Key tidak valid, tidak ditemukan, atau telah dicabut (revoked).</p>
                  </div>
                  <div className="p-6 rounded-[2rem] bg-amber-50/50 border border-amber-100">
                    <span className="text-amber-600 font-black text-sm uppercase tracking-widest">HTTP 429</span>
                    <p className="text-xs text-amber-600/80 font-bold mt-2"><code className="bg-white/50 px-1 rounded">message_limit_reached</code><br/>Kuota pesan harian Anda telah habis.</p>
                  </div>
                </div>
              </section>
            </div>
...
{`{
  ...
  "text": "Halo! Ini pesan notifikasi dari sistem."
                  ...
                  <div className="mt-6 pt-6 border-t border-slate-800">
                    <div className="text-[10px] text-slate-500 font-black uppercase tracking-widest mb-2">Response (Sukses)</div>
                    <code className="text-[12px] font-mono text-emerald-400">{`{ "ok": true, "messageId": "BAE5..." }`}</code>
                  </div>
                </div>
              </section>
              ...
              <section className="space-y-6">
                <div className="space-y-3">
                  <h2 className="text-3xl font-black text-slate-800 tracking-tight">Kirim Media (Gambar/Dokumen/Video)</h2>
                  <div className="flex items-center gap-4">
                    <span className="px-4 py-1.5 bg-emerald-100 text-emerald-600 text-[10px] font-black uppercase tracking-widest rounded-full border border-emerald-200 shadow-sm">POST</span>
...
{`{
  ...
  "caption": "Berikut adalah tagihan Anda bulan ini."
              ...
              <section className="space-y-6">
                <div className="space-y-3">
                  <h2 className="text-3xl font-black text-slate-800 tracking-tight">Kirim Lokasi Peta</h2>
                  <div className="flex items-center gap-4">
                    <span className="px-4 py-1.5 bg-emerald-100 text-emerald-600 text-[10px] font-black uppercase tracking-widest rounded-full border border-emerald-200 shadow-sm">POST</span>
                    <code className="text-sm font-bold text-slate-500">/v1/messages/send-location</code>
                  </div>
                </div>

                <div className="bg-slate-900 rounded-[2.5rem] p-8 shadow-2xl relative overflow-hidden">
                  <pre className="text-[13px] font-mono text-amber-300 overflow-x-auto leading-loose">
{`{
  "sessionKey": "device-01",
  "to": "628123456789",
  "latitude": -6.200000,
            ...
            <div className="space-y-8 animate-in slide-in-from-right-8 duration-500">
              <div className="space-y-4">
                ...
                <p className="text-sm text-slate-500 font-medium leading-relaxed max-w-3xl">
                  Sistem kami akan mengirimkan HTTP POST ke URL Webhook Anda setiap kali terjadi aktivitas pada engine Baileys kami. Berdasarkan <code className="text-indigo-500 font-bold bg-indigo-50 px-2 rounded">webhook.ts</code>, berikut adalah format yang akan Anda terima.
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="p-6 rounded-[2rem] bg-white border border-slate-100 shadow-sm">
                  ...
                  <p className="text-xs text-slate-500 leading-relaxed font-medium">Nomor WhatsApp Anda menerima pesan teks/media baru.</p>
                </div>
                <div className="p-6 rounded-[2rem] bg-white border border-slate-100 shadow-sm">
                  <div className="w-10 h-10 bg-emerald-50 text-emerald-500 rounded-xl flex items-center justify-center font-black mb-4">2</div>
                  <h3 className="font-black text-slate-800 mb-2">message.status</h3>
                  <p className="text-xs text-slate-500 leading-relaxed font-medium">Status pesan keluar berubah (<span className="italic">sent, delivered, read</span>).</p>
                </div>
                <div className="p-6 rounded-[2rem] bg-white border border-slate-100 shadow-sm">
                  <div className="w-10 h-10 bg-amber-50 text-amber-500 rounded-xl flex items-center justify-center font-black mb-4">3</div>
                  <h3 className="font-black text-slate-800 mb-2">session.update</h3>
                  <p className="text-xs text-slate-500 leading-relaxed font-medium">Sesi WA terputus atau terhubung ulang.</p>
                </div>
              </div>
              ...
              <div className="p-8 rounded-[2.5rem] bg-slate-800 text-white mt-8 shadow-xl relative overflow-hidden">
                ...
                <h3 className="text-lg font-black tracking-tight mb-4 relative z-10">Custom Headers & HMAC Signature</h3>
                <p className="text-[13px] text-slate-300 leading-relaxed font-medium relative z-10 mb-6">
                  Setiap request dari sistem kami akan membawa header wajib berikut:
                </p>
                <div className="space-y-2 relative z-10">
                  <div className="flex gap-4 items-center">
                    <code className="text-xs bg-slate-900 px-3 py-1.5 rounded-lg text-emerald-400 font-mono w-48 shrink-0">X-Webhook-Event</code>
                    <span className="text-xs text-slate-400">Tipe event (contoh: message.incoming)</span>
                  </div>
                  <div className="flex gap-4 items-center">
                    <code className="text-xs bg-slate-900 px-3 py-1.5 rounded-lg text-emerald-400 font-mono w-48 shrink-0">X-Webhook-Delivery-Id</code>
                    <span className="text-xs text-slate-400">ID unik pengiriman (untuk mencegah duplikasi)</span>
                  </div>
                  <div className="flex gap-4 items-center">
                    <code className="text-xs bg-slate-900 px-3 py-1.5 rounded-lg text-emerald-400 font-mono w-48 shrink-0">X-Webhook-Tenant</code>
                    <span className="text-xs text-slate-400">ID Tenant SaaS Anda</span>
                  </div>
                  <div className="flex gap-4 items-center pt-2">
                    <code className="text-xs bg-slate-900 border border-rose-500/30 px-3 py-1.5 rounded-lg text-rose-400 font-mono w-48 shrink-0">X-Webhook-Signature</code>
                    <span className="text-xs text-slate-400">Validasi HMAC-SHA256 dari request body menggunakan rahasia (secret) Anda.</span>
                  </div>
                </div>
              </div>
            </div>
