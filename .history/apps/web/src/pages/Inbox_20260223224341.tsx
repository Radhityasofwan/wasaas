...
export default function Inbox() {
  ...
  // Perbaikan Logika Scroll: Menggunakan scrollTop daripada scrollIntoView
  // agar tidak merusak scroll posisi kontainer induk (Shell)
  ...
  // Scroll saat pesan baru masuk
  useEffect(() => {
    scrollToBottom("smooth");
  }, [messages]);

  // Scroll instan saat ganti peer
  useEffect(() => {
    if (peer) {
  ...
  return (
    <div className="flex h-full max-h-full bg-transparent overflow-hidden rounded-[2.5rem]">
      {/* SIDEBAR */}
        ...
        <div className="flex-1 overflow-y-auto px-4 pb-4 space-y-2 scrollbar-hide">
          {filteredConvs.map(c => {
            ...
            return (
              ...
              >
                <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-blue-50 to-indigo-50 flex items-center justify-center text-blue-500 font-black text-xl shadow-sm border border-white shrink-0">
                ...
                {c.unreadCount > 0 && (
                  <div className="w-6 h-6 rounded-full bg-blue-600 text-white text-[10px] flex items-center justify-center font-black shadow-lg shadow-blue-500/30 shrink-0">
                    {c.unreadCount}
                  </div>
                )}
              </div>
      ...
      {/* CHAT VIEW */}
      {peer ? (
        <div className="flex-1 flex flex-col relative bg-white/10 min-w-0">
          {/* Chat Header */}
          <div className="h-24 px-8 flex items-center border-b border-white/20 bg-white/40 backdrop-blur-xl z-20 shrink-0">
            ...
            <div className="flex-1 min-w-0">
              <h2 className="text-lg font-black text-slate-800 tracking-tight truncate">{peer.split('@')[0]}</h2>
          ...
          {/* Chat Messages Container - Perbaikan Overflow dan Scroll */}
          <div 
            ref={scrollContainerRef}
            className="flex-1 overflow-y-auto p-10 space-y-6 scrollbar-hide scroll-smooth"
          >
          ...
          {/* Input Area */}
          <div className="p-8 bg-transparent z-20 shrink-0">
            <div className="max-w-4xl mx-auto flex items-end gap-4 bg-white/40 backdrop-blur-3xl p-3 rounded-[2.5rem] border border-white shadow-lg">
              ...
              <button 
                ...
                className={`w-14 h-14 rounded-[1.5rem] flex items-center justify-center transition-all duration-500 shrink-0 ${
        ...
        <div className="flex-1 flex flex-col items-center justify-center bg-white/5">
