...
/**
 * FEATURE: Formatter Nomor WA ke +62
 */
function formatPhoneNumber(jid: string) {
  ...
  // Jika bukan 62 (misal nomor luar negeri), tetap tambahkan +
  return `+${num}`;
}
...
export default function Inbox() {
  ...
  // Scroll otomatis ketika pesan BERTAMBAH dan user berada di bawah
  useEffect(() => {
    if (isAtBottom) {
  ...
  useEffect(() => {
    if (peer) {
      setMessages([]); // Kosongkan sementara saat ganti kontak agar tidak flicker
      setTimeout(() => {
        scrollToBottom("auto");
        setIsAtBottom(true);
      }, 100);
    ...
    <div className="flex h-full max-h-[85vh] bg-transparent overflow-hidden rounded-[2.5rem]">
      ...
      {peer ? (
        <div className="flex-1 flex flex-col relative bg-white/10 min-w-0 border-l border-white/20">
          ...
          <div className="h-24 px-8 flex items-center border-b border-white/20 bg-white/40 backdrop-blur-xl z-20 shrink-0 shadow-sm">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 mt-0.5">
                ...
                <p className="text-[10px] text-slate-400 font-bold tracking-[0.1em] uppercase">Terhubung</p>
              </div>
            </div>
          </div>
          ...
          {/* Input Area */}
          <div className="p-6 md:p-8 bg-transparent z-20 shrink-0 border-t border-white/20">
            <div className="max-w-4xl mx-auto flex items-end gap-4 bg-white/60 backdrop-blur-3xl p-3 rounded-[2rem] border border-white shadow-[0_10px_40px_rgba(0,0,0,0.03)]">
              <button 
                ...
                className="w-12 h-12 flex items-center justify-center text-slate-400 hover:text-blue-600 transition-all duration-500 hover:scale-110 active:scale-90 bg-white rounded-[1.2rem] shadow-sm"
              >
