...
export default function Inbox() {
  ...
  // Mock Label State (Digunakan agar label tampil instan di UI sebelum sync DB)
  ...
  async function executeDeleteChats() {
    if (!confirm(`Yakin ingin menghapus ${selectedPeers.length} percakapan? (Membutuhkan dukungan API di backend)`)) return;
    try {
      // Endpoint Mockup - Perlu dipastikan aktif di Backend Anda
    ...
    } catch (e: any) { alert("Sistem: API Hapus Pesan belum siap di Backend Anda."); }
  }
    ...
    try {
      await apiFetch("/broadcast/create", {
        ...
        body: JSON.stringify({
          ...
          name: `Inbox Broadcast (${bcModal.targets.length} target)`,
          targets: bcModal.targets.map(t => t.split('@')[0])
        }),
      });
      setBcModal({ open: false, targets: [] }); setBcPayload({ text: "", delay: "2000" });
      setSelectedPeers([]); setIsSelectionMode(false);
      alert("✅ Broadcast berhasil dijadwalkan dan dikirim ke Antrean!");
  ...
  async function executeSetLabel() {
    ...
    try {
      // Mocking State / E2E Prep (Bisa disambungkan ke POST /leads/label jika backend mendukung)
