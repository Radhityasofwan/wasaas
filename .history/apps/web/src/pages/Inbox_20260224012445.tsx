...
export default function Inbox() {
  ...
  // Simpan setiap kali ada perubahan label
  useEffect(() => {
    if (typeof window !== "undefined") {
  ...
  const filteredConvs = useMemo(() => {
    ...
    const cleanQ = q.replace(/\D/g, ''); // Ambil hanya angka untuk pencarian nomor
    ...
    return sortedConvs.filter(c => {
      ...
      // Filter tambahan by custom label
  ...
  useEffect(() => { loadSessions(); loadLeads(); }, []);
  ...
  // FEATURE 5 FIX: Hapus Obrolan Sync
  async function executeDeleteChats() {
    ...
    try {
      ...
      if (selectedPeers.includes(peer)) setPeer(""); // Tutup obrolan jika yang sedang dibuka ikut dihapus
      setSelectedPeers([]); 
  ...
  // FEATURE 4 FIX: Jadwalkan Broadcast Bulk
  async function executeScheduleBroadcast() {
    if (!bcPayload.text.trim()) return alert("Pesan tidak boleh kosong");
    try {
      // Ekstraksi murni hanya nomor JID untuk menghindari error loop backend
      ...
      alert(`✅ Broadcast ke ${cleanTargets.length} nomor berhasil dikirim ke antrean! Cek Menu Broadcast untuk memantau pengiriman.`);
  ...
  async function executeSetLabel() {
    ...
    try {
      ...
      // Optional: Hit ke API CRM jika backend Leads/Label nanti sudah ada
