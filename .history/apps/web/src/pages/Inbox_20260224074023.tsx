...
// ===== HELPERS =====
/**
 * FIX TIMEZONE: Normalisasi string datetime dari MySQL yang kaku
 * Mengubah "2026-02-24 00:37:00" menjadi "2026-02-24T00:37:00Z" (memaksa UTC agar sesuai standar)
 */
function normalizeDate(dateStr: string) {
  if (!dateStr) return new Date();
  
  // Ubah spasi menjadi T (Standar ISO)
  let safeStr = dateStr.replace(" ", "T");
  
  // Jika tidak memiliki indikator zona waktu ('Z' atau '+07:00'), paksa ke UTC dengan 'Z'
...
function formatChatDate(dateStr: string, nowTime: Date) {
  ...
  // Perbandingan tanggal secara absolut (mengabaikan jam) agar akurat di pergantian hari
...
/**
 * FEATURE 1: Formatter Nomor WA Cerdas & Normalisasi LID
 */
...
export default function Inbox() {
  ...
  // FEATURE: Live Clock & Auto Re-render State
  const [liveTime, setLiveTime] = useState(new Date());

  // Engine Realtime Time (Trigger Re-render per detik)
  useEffect(() => {
    const timer = setInterval(() => setLiveTime(new Date()), 1000);
    return () => clearInterval(timer);
  }, []);
  
  // FEATURE 3 FIX: Persistensi Label dengan LocalStorage
  ...
  // NEW FEATURE: Auto Follow Up Modals & State
  ...
  // FEATURE 2: Sorting Percakapan (Yang Terbaru di Atas)
  const sortedConvs = useMemo(() => {
    return [...convs].sort((a, b) => {
      const tA = new Date(normalizeDate(a.lastMessage?.time || "")).getTime();
      const tB = new Date(normalizeDate(b.lastMessage?.time || "")).getTime();
      return tB - tA; 
    });
  }, [convs]);

  // FEATURE 6: Pencarian Cerdas (Mengabaikan spasi dan simbol)
  const filteredConvs = useMemo(() => {
    if (!searchQuery) return sortedConvs;
    const q = searchQuery.toLowerCase();
    const cleanQ = q.replace(/\D/g, ''); 
    
    return sortedConvs.filter(c => {
      const numOnly = c.remoteJid.split('@')[0];
      const localNum = numOnly.startsWith('62') ? '0' + numOnly.slice(2) : numOnly;
      
      const matchString = c.remoteJid.toLowerCase().includes(q);
      const matchNumber = cleanQ && (numOnly.includes(cleanQ) || localNum.includes(cleanQ));
      
      const lbl = customLabels[numOnly]?.name.toLowerCase() || "";
      const matchLabel = lbl && lbl.includes(q);

      return matchString || matchNumber || matchLabel;
    });
  }, [sortedConvs, searchQuery, customLabels]);
  ...
  // Ambil daftar campaign follow up aktif untuk dropdown
  ...
  async function executeDeleteChats() {
    ...
    try {
      ...
      alert("Sistem: Pastikan endpoint /ui/conversations/delete sudah ditambahkan di backend. Error: " + e.message); 
    }
  }
  ...
  async function executeScheduleBroadcast() {
    ...
    try {
      ...
      await apiFetch("/broadcast/create", {
        method: "POST",
        body: JSON.stringify({
          sessionKey, text: bcPayload.text, delayMs: Number(bcPayload.delay),
  ...
  async function executeSetLabel() {
    ...
    try {
      ...
      apiFetch("/leads/label", {
         method: "POST",
         body: JSON.stringify({ targets: labelModal.targets.map(t=>t.split('@')[0]), label: labelPayload.name, color: labelPayload.color })
      }).catch(() => {});
  ...
  return (
    <div className="flex h-full max-h-[85vh] bg-transparent overflow-hidden rounded-[2.5rem] relative">
      ...
      <div className="w-full md:w-[350px] lg:w-[400px] flex flex-col border-r border-white/20 bg-white/30 backdrop-blur-3xl shrink-0 relative z-10">
        ...
        {/* Sidebar Header dengan Live Clock Indicator */}
        <div className="h-24 px-8 flex items-center justify-between border-b border-white/20 shrink-0">
          ...
          <button 
            ...
            title="Mode Pilihan (Bulk Action)"
          >
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="9 11 12 14 22 4"></polyline><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"></path></svg>
          </button>
        </div>
        ...
        <div className="p-6 shrink-0">
          <div className="relative">
            <input 
              ...
              placeholder="Cari nomor atau label..."
          ...
          {filteredConvs.map(c => {
            ...
            return (
              <div 
                ...
                className={`p-4 flex items-center gap-4 rounded-[2rem] cursor-pointer transition-all duration-300 ${
                  isActive ? "bg-white/90 shadow-lg shadow-blue-500/10 border border-white scale-[1.02]" :
                  isSelected ? "bg-blue-50 border border-blue-200" : "hover:bg-white/40 border border-transparent"
                }`}
              >
                {/* Bulk Checkbox / Avatar */}
                {isSelectionMode ? (
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 border-2 transition-all ${isSelected ? 'bg-blue-600 border-blue-600 text-white' : 'bg-white border-slate-200'}`}>
                    {isSelected && <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="4"><polyline points="20 6 9 17 4 12"></polyline></svg>}
                  </div>
                ) : (
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center font-black text-lg shrink-0 border border-white shadow-sm ${
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-center mb-1">
                    <h3 className="text-[14px] font-extrabold text-slate-800 truncate tracking-tight">{formatPhoneNumber(c.remoteJid)}</h3>
                    {/* WAKTU REALTIME TER-NORMALISASI */}
                    <span className="text-[9px] text-slate-400 font-bold uppercase tracking-tighter shrink-0">{formatChatDate(c.lastMessage?.time, liveTime)}</span>
                  </div>
                  
                  <div className="flex items-center gap-2 mb-1">
                    {/* CUSTOM LABEL UI */}
                    {lLabel && (
                      <span className={`px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-widest text-white shadow-sm ${lLabel.color}`}>
                        {lLabel.name}
                      </span>
                    )}
                    {isLead && (
                  ...
                  <p className="text-[12px] text-slate-500 font-medium truncate opacity-80 mt-0.5">
                    {c.lastMessage?.direction === 'out' && <span className="text-blue-500 font-black mr-1">✓</span>}
                    {c.lastMessage?.text || '[Media]'}
                  </p>
                </div>

                {c.unreadCount > 0 && !isSelectionMode && (
                  <div className="w-5 h-5 rounded-full bg-rose-500 text-white text-[9px] flex items-center justify-center font-black shadow-lg shrink-0">
                    {c.unreadCount}
                  </div>
                )}
              </div>
        ...
        {isSelectionMode && selectedPeers.length > 0 && (
          <div className="absolute bottom-4 left-4 right-4 bg-slate-900 rounded-[2rem] p-4 shadow-2xl border border-slate-700 flex items-center justify-between animate-in slide-in-from-bottom-6 z-50">
             ...
             <div className="flex gap-2">
                <button onClick={() => setLabelModal({ open: true, targets: selectedPeers })} className="p-2.5 bg-slate-800 hover:bg-indigo-600 rounded-xl text-white transition-colors" title="Beri Label"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M20.59 13.41l-7.17 7.17a2 2 0 0 1-2.83 0L2 12V2h10l8.59 8.59a2 2 0 0 1 0 2.82z"></path><line x1="7" y1="7" x2="7.01" y2="7"></line></svg></button>
                ...
                <button onClick={executeDeleteChats} className="p-2.5 bg-slate-800 hover:bg-rose-600 rounded-xl text-white transition-colors" title="Hapus"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg></button>
             </div>
          </div>
        )}
      </div>
      ...
      {peer ? (
        <div className="flex-1 flex flex-col relative bg-white/10 min-w-0 border-l border-white/20">
          ...
          <div className="h-24 px-8 flex items-center border-b border-white/20 bg-white/40 backdrop-blur-xl z-20 shrink-0 shadow-sm justify-between">
            ...
            {/* Aksi Tunggal Obrolan */}
            <div className="flex gap-2">
              <button onClick={() => setLabelModal({ open: true, targets: [peer] })} className="w-10 h-10 rounded-xl bg-white text-slate-500 hover:text-indigo-600 flex items-center justify-center shadow-sm border border-white transition-all hover:scale-110" title="Ubah Label"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M20.59 13.41l-7.17 7.17a2 2 0 0 1-2.83 0L2 12V2h10l8.59 8.59a2 2 0 0 1 0 2.82z"></path><line x1="7" y1="7" x2="7.01" y2="7"></line></svg></button>
          ...
          <div className="p-6 md:p-8 bg-transparent z-20 shrink-0 border-t border-white/20 relative">
            ...
            {attachOpen && (
              <div className="absolute bottom-28 left-8 bg-white/95 backdrop-blur-xl border border-white/80 p-4 rounded-[2rem] shadow-2xl flex flex-col gap-2 z-50 animate-in slide-in-from-bottom-4 min-w-[220px]">
                ...
                <button onClick={() => { setMediaModal({ open: true, type: 'location' }); setAttachOpen(false); }} className="flex items-center gap-4 px-4 py-3 hover:bg-slate-50 rounded-2xl text-slate-700 font-bold transition-all text-sm w-full text-left">
                  <span className="text-amber-500 text-xl drop-shadow-sm">📍</span> Bagikan Titik Lokasi
                </button>
              </div>
            )}
      ...
      {labelModal.open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-xl animate-in fade-in">
          <div className="w-full max-w-sm bg-white rounded-[2.5rem] p-8 shadow-2xl border border-white">
            ...
            <input value={labelPayload.name} onChange={(e)=>setLabelPayload({...labelPayload, name: e.target.value})} placeholder="Contoh: VIP Customer" className="w-full px-5 py-4 rounded-2xl bg-slate-50 border border-slate-200 outline-none font-bold text-slate-700 mb-4" />
            ...
            <div className="flex gap-3 justify-end">
              ...
              <button onClick={executeSetLabel} className="px-6 py-3 rounded-xl font-bold text-white bg-blue-600 text-xs uppercase tracking-widest shadow-lg shadow-blue-500/30">Simpan Label</button>
            </div>
          </div>
        </div>
      )}

      {/* 2. Modal Broadcast Terintegrasi */}
      {bcModal.open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-xl animate-in fade-in">
          <div className="w-full max-w-lg bg-white rounded-[3rem] p-10 shadow-2xl border border-white">
            <h3 className="text-3xl font-black text-slate-800 tracking-tight mb-1">Jadwalkan Broadcast</h3>
            <p className="text-xs font-bold text-slate-400 mb-6 uppercase tracking-widest">{bcModal.targets.length} Penerima Terpilih</p>
            
            <textarea rows={5} value={bcPayload.text} onChange={(e)=>setBcPayload({...bcPayload, text: e.target.value})} placeholder="Ketik pesan penawaran / broadcast..." className="w-full px-6 py-4 rounded-[2rem] bg-slate-50 border border-slate-200 outline-none font-bold text-slate-700 mb-4 resize-none" />
            <div className="flex items-center justify-between bg-slate-50 px-6 py-4 rounded-2xl border border-slate-100 mb-8">
               <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Jeda per Pesan (Ms)</span>
               <input value={bcPayload.delay} onChange={(e)=>setBcPayload({...bcPayload, delay: e.target.value})} className="w-20 text-right bg-transparent outline-none font-black text-slate-800" />
            </div>

            <div className="flex gap-3 justify-end">
              <button onClick={() => setBcModal({ open: false, targets: [] })} className="px-8 py-4 rounded-2xl font-black text-slate-500 bg-slate-100 text-[10px] uppercase tracking-widest">Batal</button>
              <button onClick={executeScheduleBroadcast} className="px-8 py-4 rounded-2xl font-black text-white bg-emerald-500 hover:bg-emerald-600 text-[10px] uppercase tracking-widest shadow-lg shadow-emerald-500/30 transition-all">Mulai Broadcast</button>
            </div>
          </div>
        </div>
      )}

      {/* 3. Modal Kirim Media (URL) */}
      {mediaModal.open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-xl animate-in fade-in">
          <div className="w-full max-w-md bg-white rounded-[2.5rem] p-8 shadow-2xl border border-white">
            <h3 className="text-xl font-black text-slate-800 tracking-tight mb-6">
              {mediaModal.type === 'location' ? 'Kirim Titik Lokasi' : `Kirim URL ${mediaModal.type === 'image' ? 'Gambar' : 'Dokumen'}`}
            </h3>
            
            {mediaModal.type === 'location' ? (
              <div className="grid grid-cols-2 gap-4 mb-6">
                 <input value={mediaPayload.lat} onChange={(e)=>setMediaPayload({...mediaPayload, lat: e.target.value})} placeholder="Latitude" className="w-full px-5 py-4 rounded-2xl bg-slate-50 border border-slate-200 outline-none font-bold text-slate-700" />
                 <input value={mediaPayload.lng} onChange={(e)=>setMediaPayload({...mediaPayload, lng: e.target.value})} placeholder="Longitude" className="w-full px-5 py-4 rounded-2xl bg-slate-50 border border-slate-200 outline-none font-bold text-slate-700" />
              </div>
            ) : (
              <div className="space-y-4 mb-6">
                <input value={mediaPayload.url} onChange={(e)=>setMediaPayload({...mediaPayload, url: e.target.value})} placeholder="https://domain.com/file.jpg" className="w-full px-5 py-4 rounded-2xl bg-slate-50 border border-slate-200 outline-none font-bold text-slate-700" />
                <input value={mediaPayload.caption} onChange={(e)=>setMediaPayload({...mediaPayload, caption: e.target.value})} placeholder="Caption (Opsional)" className="w-full px-5 py-4 rounded-2xl bg-slate-50 border border-slate-200 outline-none font-bold text-slate-700" />
              </div>
            )}

            <div className="flex gap-3 justify-end">
              <button onClick={() => setMediaModal({ open: false, type: 'image' })} className="px-6 py-3 rounded-xl font-bold text-slate-500 bg-slate-100 text-xs uppercase tracking-widest">Batal</button>
              <button onClick={executeSendMedia} disabled={sending} className="px-6 py-3 rounded-xl font-bold text-white bg-blue-600 text-xs uppercase tracking-widest shadow-lg shadow-blue-500/30">{sending ? 'Mengirim...' : 'Kirim Sekarang'}</button>
            </div>
          </div>
        </div>
      )}

      {/* 4. NEW: Modal Tambah Target Follow Up */}
      {fuModal.open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-xl animate-in fade-in">
          <div className="w-full max-w-lg bg-white rounded-[3rem] p-10 shadow-2xl border border-white">
            <h3 className="text-3xl font-black text-slate-800 tracking-tight mb-1">Auto Follow Up</h3>
            <p className="text-xs font-bold text-slate-400 mb-6 uppercase tracking-widest">{fuModal.targets.length} Penerima Terpilih</p>
            
            <div className="mb-8">
              <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest block mb-2">Pilih Campaign Follow Up</label>
              {campaigns.length > 0 ? (
                <select 
                  value={fuPayload.campaignId} 
                  onChange={(e)=>setFuPayload({...fuPayload, campaignId: e.target.value})} 
                  className="w-full px-6 py-4 rounded-[2rem] bg-slate-50 border border-slate-200 outline-none font-bold text-slate-700 appearance-none cursor-pointer"
                >
                  {campaigns.map(c => (
                ...
                <div className="px-6 py-4 rounded-[1.5rem] bg-rose-50 border border-rose-100 text-rose-500 text-sm font-bold leading-relaxed">
                  Belum ada Campaign Follow Up yang aktif. Anda harus membuatnya terlebih dahulu melalui menu "Follow Up" di Sidebar.
                </div>
              )}
            </div>

            <div className="flex gap-3 justify-end">
              <button onClick={() => setFuModal({ open: false, targets: [] })} className="px-8 py-4 rounded-2xl font-black text-slate-500 bg-slate-100 text-[10px] uppercase tracking-widest">Batal</button>
              <button 
                onClick={executeAddToFollowUp} 
                disabled={campaigns.length === 0} 
                className="px-8 py-4 rounded-2xl font-black text-white bg-orange-500 hover:bg-orange-600 disabled:bg-slate-300 disabled:cursor-not-allowed text-[10px] uppercase tracking-widest shadow-lg shadow-orange-500/30 transition-all"
              >
                Tambahkan Target
              </button>
            </div>
