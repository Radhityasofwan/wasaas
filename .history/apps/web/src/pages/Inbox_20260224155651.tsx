...
type ConvRow = {
  chatId: number;
  remoteJid: string;
  name?: string; // FIX: Ditambahkan untuk menangkap nama dari backend
  unreadCount: number;
...
// ===== HELPERS (TIMEZONE BUG FIX) =====

function normalizeDate(dateStr: string) {
  if (!dateStr) return new Date();
  
  // Tangkap string tanggal dari API
  let safeStr = dateStr;
  ...
  // FIX: KOMPENSASI BUG TIMEZONE MYSQL (UTC) VS NODE.JS (WIB)
...
/**
 * FEATURE 1: Formatter Nama/Nomor WA Cerdas
 * FIX: Memprioritaskan nama kontak jika tersedia dari database.
 */
function formatContactName(jid: string, name?: string) {
  if (name && name.trim() !== "") return name; // Gunakan nama jika ada
...
export default function Inbox() {
  ...
  const filteredConvs = useMemo(() => {
    ...
    return sortedConvs.filter(c => {
      ...
      return matchString || matchName || matchNumber || matchLabel;
    });
              ...
              >
                ...
                {isSelectionMode ? (
                  ...
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center font-black text-lg shrink-0 border border-white shadow-sm ${
                    ...
                    {(c.name ? c.name.charAt(0) : c.remoteJid.charAt(0)).toUpperCase()}
                  </div>
                )}

                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-center mb-1">
                    <h3 className="text-[14px] font-extrabold text-slate-800 truncate tracking-tight">{formatContactName(c.remoteJid, c.name)}</h3>
                    {/* WAKTU REALTIME TER-NORMALISASI KOMPENSASI TIMEZONE */}
