  ...
  async function loadMessages(sk: string, p: string, limit: number) {
    try {
      ...
      setMessages(prev => {
        ...
        // Cegah re-render jika data masih sama identik
          ...
          {filteredConvs.map(c => {
            ...
            return (
              <div 
                ...
                className={`p-4 flex items-center gap-4 rounded-[1.5rem] cursor-pointer transition-all duration-300 relative ${
                  isActive 
                    ? "bg-white/95 shadow-lg shadow-blue-500/10 border border-white scale-[1.02]" 
                    : isSelected 
              ...
              >
                ...
                {isSelectionMode ? (
                  <div className={`w-12 h-12 rounded-[1rem] flex items-center justify-center shrink-0 border-2 transition-all ${
                    isSelected 
                  ...
                  <div className={`w-12 h-12 rounded-[1rem] flex items-center justify-center font-black text-xl shrink-0 border border-white shadow-sm relative ${
                    isActive 
                    ...
                    {/* Unread dot indicator floating on avatar */}
                    {isUnread && !isActive && (
                ...
                <div className="flex-1 min-w-0 pr-6">
                  ...
                  <div className="flex justify-between items-center mb-0.5">
                    {/* UI UX FIX: Black Bold untuk Unread, Medium untuk Read */}
                    <h3 className={`text-[15px] truncate tracking-tight ${
                      isUnread 
                    ...
                    <span className={`text-[10px] font-bold uppercase tracking-tighter shrink-0 absolute right-4 top-4 ${
                  ...
                  <div className="flex flex-wrap items-center gap-1.5 mb-1.5">
                    {lLabel && (
                      <span className={`px-1.5 py-0.5 rounded-md text-[9px] font-black uppercase tracking-widest text-white shadow-sm flex items-center gap-1 ${lLabel.color}`}>
                        {lLabel.name} 
                        {/* Tombol silang khusus untuk menghapus label dengan instan */}
                        {isActive && (
                          <span 
                  ...
                  {/* Teks Pratinjau Pesan Terakhir */}
                  <p className={`text-[13px] truncate pr-4 ${
                    isUnread 
                      ? 'text-slate-800 font-bold' 
                      : 'text-slate-500 font-medium opacity-80'
                  }`}>
                    {c.lastMessage?.direction === 'out' && <span className="text-blue-500 font-black mr-1.5">✓</span>}
                    {c.lastMessage?.text || '[Media Lampiran]'}
                  </p>
                </div>

                {/* Indikator Hitungan Unread (Kanan Bawah) */}
                {isUnread && !isSelectionMode && (
                  <div className="absolute right-4 bottom-4 w-6 h-6 rounded-full bg-emerald-500 text-white text-[10px] flex items-center justify-center font-black shadow-md shrink-0 ring-4 ring-emerald-50">
                    {c.unreadCount}
                  </div>
                )}
              </div>
      ...
      {peer ? (
        <div className="flex-1 flex flex-col relative bg-white/10 min-w-0 border-l border-white/20">
          ...
          >
            ...
            {/* UI/UX FIX (CRITICAL): 
                Fungsi .reverse() telah dihapus dari pemetaan.
                Karena API listMessages dari backend (ui_routes.ts) sudah membalikkannya
                (sehingga array[0] adalah pesan tertua dan array[length-1] adalah terbaru),
                melakukan map secara normal akan menampilkan pesan terbaru di posisi PALING BAWAH, 
                persis seperti UI standar WhatsApp sesungguhnya!
            */}
            {messages.map((m) => {
