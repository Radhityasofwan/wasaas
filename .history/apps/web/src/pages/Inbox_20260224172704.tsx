...
export default function Inbox() {
  ...
  // Dipakai HANYA untuk kebutuhan label dan deteksi leads (yang disimpan murni by number)
  ...
  async function loadMessages(sk: string, p: string, limit: number) {
    try {
      // FIX PENTING: JANGAN DI-SPLIT! 
      // Kirim parameter JID utuh (termasuk @g.us / @s.whatsapp.net / @lid) 
      // agar backend tidak gagal query dan memunculkan bubble chat.
      ...
      setMessages(prev => {
        ...
        // FIX RENDER OPTIMIZATION: 
        // Jika panjang pesan berbeda (ada pesan baru), langsung ganti ke array baru
        if (prev.length !== newMsgs.length) return newMsgs;
        if (prev.length === 0) return newMsgs;
        
        // Cek ID & Status pada elemen TERAKHIR (terbaru), bukan elemen pertama (tertua)!
        ...
        // Loop verifikasi ekstra untuk status pesan di tengah antrean
        let hasChanges = false;
        for (let i = 0; i < prev.length; i++) {
          if (prev[i].status !== newMsgs[i].status) {
            hasChanges = true;
            break;
          }
        }
  ...
  async function sendText() {
    ...
    try {
      await apiFetch(`/messages/send`, {
        method: "POST",
        // FIX: Gunakan "peer" utuh, jangan di-trim!
        body: JSON.stringify({ sessionKey, to: peer, text: text.trim() }),
      });
      setText("");
      
      // Tunggu muat ulang selesai baru scroll down
  ...
  async function executeSendMedia() {
    ...
    try {
      ...
      // FIX: Gunakan "peer" agar support Grup/LID
      const payload = isLoc 
      ...
      {peer ? (
        <div className="flex-1 flex flex-col relative bg-white/10 min-w-0 border-l border-white/20">
          ...
          >
            ...
            {/* UI/UX FIX: Array dipetakan langsung (sudah di-reverse dari Backend).
                Menghasilkan urutan pesan natural (pesan baru masuk selalu di paling bawah) */}
            {messages.map((m) => {
