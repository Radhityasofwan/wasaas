...
type MsgRow = {
  id: number; direction: "in" | "out"; type: string; text: string | null; status: string; time: string; 
};
...
export default function Leads() {
  ...
  const handleExportCSV = () => {
    ...
    const csvContent = [
      ...
      ...leads.map(l => [
        `"${(l.name || "").replace(/"/g, '""')}"`,
        l.to_number, l.source, l.status, l.total_broadcasts, l.total_followups, l.last_interacted_at || "-"
      ].join(","))
    ].join("\n");
  ...
  const executeRetargetBroadcast = async () => {
    ...
    try {
      await apiFetch("broadcast/create", {
        method: "POST",
        body: JSON.stringify({
          sessionKey: bcPayload.sessionKey, text: bcPayload.text, delayMs: Number(bcPayload.delay),
          name: `Retargeting Leads (${selectedLeads.length} Target)`, targets: selectedLeads
        })
      });
      setBcModalOpen(false); setSelectedLeads([]); setIsSelectionMode(false);
  ...
  const executeRetargetFollowUp = async () => {
    ...
    try {
      await apiFetch("followup/add-targets", {
        method: "POST",
        body: JSON.stringify({ sessionKey: fuPayload.sessionKey, campaignId: fuPayload.campaignId, targets: selectedLeads })
      });
      setFuModalOpen(false); setSelectedLeads([]); setIsSelectionMode(false);
  ...
  const sendQuickChat = async () => {
    ...
    try {
       ...
       await apiFetch("messages/send", { 
         method: "POST", body: JSON.stringify({ sessionKey: chatSession, to: peerJid, text: chatText.trim() })
       });
       setChatText("");
  ...
  const executeDeleteLeads = async (targets: string[]) => {
    if (!confirm(`TINDAKAN BERBAHAYA: Anda yakin ingin menghapus permanen ${targets.length} Prospek ini dari Master CRM?\n\n(Aksi ini tidak dapat dibatalkan)`)) return;
    try {
      await apiFetch("leads/delete", {
        method: "POST",
      ...
      loadData();
      alert(`Berhasil! ${targets.length} Prospek telah dihapus.`);
  ...
  const parseTags = (tagsJson: any) => {
    ...
    try {
      ...
      if (t && t.name && t.color) return <span className={`px-2 py-0.5 rounded text-[9px] font-black text-white ${t.color}`}>{t.name}</span>;
    } catch(e) {}
    return null;
  };
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700 pb-20">
      ...
      <div className="bg-white/40 backdrop-blur-3xl border border-white rounded-[3rem] overflow-hidden shadow-sm relative">
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-white/30 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-white/50">
                ...
                <th className="px-6 py-6 text-center">Interaksi Sistem</th>
                ...
                >
                  ...
                  <td className="px-6 py-5 text-center align-middle">
                    <div className="inline-flex gap-2">
                      <div className="flex flex-col items-center p-2 bg-white rounded-xl border border-slate-100 shadow-sm min-w-[50px]" title="Jumlah dikirimi Broadcast">
                      ...
                      <div className="flex flex-col items-center p-2 bg-white rounded-xl border border-slate-100 shadow-sm min-w-[50px]" title="Jumlah dimasukkan ke Auto Follow Up">
                  ...
                  <td className="px-6 py-5 align-middle text-right">
                    <div className="flex justify-end gap-1.5 opacity-40 hover:opacity-100 focus-within:opacity-100 transition-opacity">
                      <button onClick={() => openWaMe(lead.to_number)} className="w-9 h-9 rounded-xl bg-white text-blue-600 flex items-center justify-center hover:bg-blue-50 border border-slate-200 transition-all shadow-sm" title="Chat Manual di WA Web"><ExternalLink size={16}/></button>
                      <button onClick={() => { setSelectedLeads([lead.to_number]); setBcModalOpen(true); }} className="w-9 h-9 rounded-xl bg-white text-emerald-600 flex items-center justify-center hover:bg-emerald-50 border border-slate-200 transition-all shadow-sm" title="Kirim Broadcast Khusus"><Megaphone size={16}/></button>
                      <button onClick={() => { setSelectedLeads([lead.to_number]); setFuModalOpen(true); }} className="w-9 h-9 rounded-xl bg-white text-orange-500 flex items-center justify-center hover:bg-orange-50 border border-slate-200 transition-all shadow-sm" title="Jadwalkan Follow Up"><CalendarClock size={16}/></button>
                      <button onClick={() => executeDeleteLeads([lead.to_number])} className="w-9 h-9 rounded-xl bg-white text-rose-500 flex items-center justify-center hover:bg-rose-50 border border-slate-200 transition-all shadow-sm" title="Hapus Lead Permanen"><Trash2 size={16}/></button>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        {isSelectionMode && selectedLeads.length > 0 && (
          <div className="absolute bottom-6 left-1/2 -translate-x-1/2 bg-slate-900 rounded-[2rem] p-4 shadow-2xl border border-slate-700 flex items-center gap-6 animate-in slide-in-from-bottom-10 z-50">
             <span className="text-[11px] font-black text-white px-4 py-2 bg-slate-800 rounded-xl border border-slate-700">{selectedLeads.length} Terpilih</span>
             <div className="flex gap-2">
                <button onClick={() => setBcModalOpen(true)} className="flex items-center gap-2 px-4 py-2 bg-emerald-500 hover:bg-emerald-600 rounded-xl text-white font-black text-[10px] uppercase tracking-widest transition-all"><Megaphone size={14} /> Blast</button>
                <button onClick={() => setFuModalOpen(true)} className="flex items-center gap-2 px-4 py-2 bg-orange-500 hover:bg-orange-600 rounded-xl text-white font-black text-[10px] uppercase tracking-widest transition-all"><CalendarClock size={14} /> Follow Up</button>
                <div className="w-[2px] h-6 bg-slate-700 mx-1 self-center rounded-full"></div>
                <button onClick={() => executeDeleteLeads(selectedLeads)} className="flex items-center gap-2 px-4 py-2 bg-rose-500 hover:bg-rose-600 rounded-xl text-white font-black text-[10px] uppercase tracking-widest transition-all"><Trash2 size={14} /> Hapus</button>
             </div>
          </div>
        )}
      </div>
      ...
      {/* ============================================================================ */}
      {/* QUICK CHAT MODAL & RETARGETING MODALS */}
      {/* ============================================================================ */}
          ...
          <div className="w-full max-w-lg bg-white rounded-[3rem] p-8 shadow-2xl border border-slate-100 animate-in zoom-in-95">
            ...
            <div className="space-y-5 mb-8">
              ...
              <div>
                ...
                <div className="border border-slate-200 rounded-[1.5rem] bg-slate-50 overflow-hidden shadow-inner">
                  <div className="px-4 py-3 border-b border-slate-200 bg-slate-100/80 flex justify-between items-center"><span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">👀 Pratinjau Pesan</span><button type="button" onClick={() => setBcPreviewTrigger(p => p + 1)} className="text-[9px] font-bold bg-white border border-slate-200 text-slate-600 px-3 py-1.5 rounded-lg flex items-center gap-1.5 hover:text-emerald-600 hover:border-emerald-200 transition-colors shadow-sm cursor-pointer active:scale-95"><RefreshCw size={12} /> Acak</button></div>
                  <div className="p-5"><div className="bg-white rounded-tr-2xl rounded-tl-2xl rounded-br-2xl rounded-bl-sm p-4 text-[14px] font-medium text-slate-700 shadow-sm border border-slate-100 whitespace-pre-wrap leading-relaxed max-w-[85%]">{parsedBcPreview || <span className="text-slate-400 italic">Ketik sesuatu untuk melihat hasil akhir pesan...</span>}</div></div>
                </div>
              </div>
            </div>
