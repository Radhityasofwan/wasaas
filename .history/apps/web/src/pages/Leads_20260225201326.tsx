...
export default function Leads() {
  ...
  // LIVE PREVIEW PARSER UNTUK BROADCAST MODAL
  ...
  // Konfigurasi Warna untuk Dropdown Status
      ...
      <div className="bg-white/40 backdrop-blur-3xl border border-white rounded-[3rem] overflow-hidden shadow-sm relative">
              ...
              {loading ? (
                ...
                <tr 
                  ...
                  onClick={(e) => {
                    ...
                    // Cegah popup chat memicu jika yang diklik adalah input select / button
                ...
                >
                  ...
                  <td className="px-6 py-5 align-middle text-right">
                    <div className="flex justify-end gap-1.5 opacity-40 hover:opacity-100 focus-within:opacity-100 transition-opacity">
                      ...
                      <button onClick={() => executeDeleteLeads([lead.to_number])} className="w-9 h-9 rounded-xl bg-white text-rose-500 flex items-center justify-center hover:bg-rose-50 border border-slate-200 transition-all shadow-sm" title="Hapus Lead Permanen">
                        <Trash2 size={16}/>
                      </button>
                    </div>
                  </td>
        ...
        {isSelectionMode && selectedLeads.length > 0 && (
          <div className="absolute bottom-6 left-1/2 -translate-x-1/2 w-max bg-slate-900 rounded-[2rem] p-4 shadow-2xl border border-slate-700 flex items-center gap-6 animate-in slide-in-from-bottom-10 z-50">
             <span className="text-[11px] font-black text-white px-4 py-2 bg-slate-800 rounded-xl border border-slate-700 whitespace-nowrap">
               {selectedLeads.length} Prospek Terpilih
             </span>
             <div className="flex items-center gap-2">
                
                {/* [NEW] UBAH SUHU MASSAL */}
                <div className="relative group flex items-center bg-slate-800 rounded-xl border border-slate-700 mr-2 px-1">
                  ...
                  <select 
                    onChange={(e) => {
                      if(e.target.value) {
                         executeUpdateStatus(selectedLeads, e.target.value);
                         e.target.value = ""; // Reset setelah action
                      }
                    }} 
                    className="bg-transparent text-[10px] font-black text-white uppercase tracking-widest pl-2 pr-8 py-2.5 outline-none cursor-pointer appearance-none"
                  >
                    <option value="" className="text-slate-800">-- Ubah Suhu --</option>
                    <option value="hot" className="text-slate-800">🔥 Set Hot</option>
                    <option value="warm" className="text-slate-800">☀️ Set Warm</option>
                    <option value="cold" className="text-slate-800">❄️ Set Cold</option>
                    <option value="converted" className="text-slate-800">✅ Set Converted</option>
                    <option value="dead" className="text-slate-800">❌ Set Dead</option>
                  </select>
                  <ChevronDown size={12} className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none text-slate-400" />
                </div>

                <button onClick={() => setBcModalOpen(true)} className="flex items-center gap-2 px-5 py-2.5 bg-emerald-500 hover:bg-emerald-600 rounded-xl text-white font-black text-[10px] uppercase tracking-widest transition-all">
                  <Megaphone size={14} /> Blast Promo
                </button>
                <button onClick={() => setFuModalOpen(true)} className="flex items-center gap-2 px-5 py-2.5 bg-orange-500 hover:bg-orange-600 rounded-xl text-white font-black text-[10px] uppercase tracking-widest transition-all">
                  <CalendarClock size={14} /> Auto Follow Up
                </button>
                <div className="w-[2px] h-6 bg-slate-700 mx-1 self-center rounded-full"></div>
                <button onClick={() => executeDeleteLeads(selectedLeads)} className="flex items-center gap-2 px-4 py-2.5 bg-rose-500 hover:bg-rose-600 rounded-xl text-white font-black text-[10px] uppercase tracking-widest transition-all">
              ...
              <div>
                ...
                <div className="mt-3 mb-5 bg-emerald-50/50 border border-emerald-100 rounded-2xl p-4">
                  <div className="flex flex-wrap gap-2">
                    <button onClick={() => setBcPayload(p => ({...p, text: p.text + '{{nama}}'}))} className="px-3 py-1.5 bg-white border border-emerald-200 text-emerald-600 rounded-lg text-xs font-bold shadow-sm hover:bg-emerald-600 hover:text-white transition-colors">
                      + {"{{nama}}"}
                    </button>
                    <button onClick={() => setBcPayload(p => ({...p, text: p.text + '{{nomor}}'}))} className="px-3 py-1.5 bg-white border border-emerald-200 text-emerald-600 rounded-lg text-xs font-bold shadow-sm hover:bg-emerald-600 hover:text-white transition-colors">
                      + {"{{nomor}}"}
                    </button>
                    <button onClick={() => setBcPayload(p => ({...p, text: p.text + 'Selamat {{salam}}'}))} className="px-3 py-1.5 bg-white border border-emerald-200 text-emerald-600 rounded-lg text-xs font-bold shadow-sm hover:bg-emerald-600 hover:text-white transition-colors">
                      + {"{{salam}}"}
                    </button>
                    <button onClick={() => setBcPayload(p => ({...p, text: p.text + '{Halo|Hai|Permisi}'}))} className="px-3 py-1.5 bg-white border border-emerald-200 text-emerald-600 rounded-lg text-xs font-bold shadow-sm hover:bg-emerald-600 hover:text-white transition-colors">
                      Spintax {"{A|B}"}
                    </button>
                  </div>
                </div>
                ...
                <div className="border border-slate-200 rounded-[1.5rem] bg-slate-50 overflow-hidden shadow-inner">
                  <div className="px-4 py-3 border-b border-slate-200 bg-slate-100/80 flex justify-between items-center">
                    ...
                    <button type="button" onClick={() => setBcPreviewTrigger(p => p + 1)} className="text-[9px] font-bold bg-white border border-slate-200 text-slate-600 px-3 py-1.5 rounded-lg flex items-center gap-1.5 hover:text-emerald-600 hover:border-emerald-200 transition-colors shadow-sm cursor-pointer active:scale-95">
                  ...
                  <div className="p-5">
                    <div className="bg-white rounded-tr-2xl rounded-tl-2xl rounded-br-2xl rounded-bl-sm p-4 text-[14px] font-medium text-slate-700 shadow-sm border border-slate-100 whitespace-pre-wrap leading-relaxed max-w-[85%]">
                      {parsedBcPreview || <span className="text-slate-400 italic">Ketik sesuatu untuk melihat hasil akhir pesan...</span>}
                    </div>
                  </div>
                </div>
