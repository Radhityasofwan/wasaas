...
import { 
  ...
  MonitorPlay, Camera, Globe, Hash, Settings2, Info, HelpCircle
} from "lucide-react";
...
export default function Leads() {
  ...
  const isLidRef = useRef<boolean>(false); // FIX @lid 
  ...
  // Auto Polling Data Leads setiap 10 detik agar realtime
  useEffect(() => { 
    loadData(true); 
  ...
  // Quick Chat Polling Engine with @LID Dual-Fetch Fix
  useEffect(() => {
    let intervalId: any;
    if (chatModal.open && chatModal.lead && chatSession) {
      const fetchChat = async () => {
        try {
          ...
          // Coba fetch keduanya untuk mengakomodasi device companion meta
          ...
          // Jika pesan ditemukan di LID, set tracker agar balasan via modal menggunakan LID
  ...
  // UI Formatters...
  const handleExportCSV = () => { /* Logic Export CSV */ 
    ...
    const csvContent = [
      headers.join(","),
      ...leads.map(l => [`"${(l.name || "").replace(/"/g, '""')}"`, l.to_number, l.source, l.status, l.total_broadcasts, l.total_followups, l.last_interacted_at || "-"].join(","))
    ].join("\n");
  ...
  const sendQuickChat = async () => { 
    ...
    try {
       const cleanNum = chatModal.lead.to_number.replace(/\D/g, '');
       // Menggunakan LID jika sebelumnya API menangkap dari LID
