...
export default function Leads() {
  ...
  // ============================================================================
  // EXPORT NATIVE CSV ENGINE
  // ============================================================================
  const executeExportNative = async () => {
    setExporting(true);
    try {
      const qs = new URLSearchParams();
      ...
      const headers = [
        "Nama Kontak", "Nomor WhatsApp", "Sumber Trafik", "Suhu / Status", 
        "Total Di-Broadcast", "Total Masuk Follow Up", "Terakhir Interaksi", "Tanggal Masuk (Dibuat)"
      ];

      const csvContent = [
        headers.join(","),
        ...dataToExport.map((l: any) => {
          let sourceLabel = l.source;
          if (l.source.startsWith('meta_ads')) sourceLabel = l.source.replace('|', ' - ');

          return [
            `"${(l.name || "Pelanggan Baru").replace(/"/g, '""')}"`,
            `"${l.to_number}"`,
            `"${sourceLabel.toUpperCase()}"`,
            `"${l.status.toUpperCase()}"`,
            l.total_broadcasts,
            l.total_followups,
            `"${l.last_interacted_at ? new Date(l.last_interacted_at).toLocaleString('id-ID') : "-"}"`,
            `"${new Date(l.created_at).toLocaleString('id-ID')}"`
          ].join(",");
        })
      ].join("\n");

      // Menggunakan BOM (\uFEFF) agar file CSV terbuka rapi di Microsoft Excel
      const blob = new Blob(["\uFEFF" + csvContent], { type: 'text/csv;charset=utf-8;' });
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `Export_Leads_${exportPayload.source}_${exportPayload.status}_${new Date().toISOString().slice(0,10)}.csv`;
      a.click();
    ...
    <div className="space-y-8 animate-in fade-in duration-700 pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex gap-3">
          ...
          >
            <FileSpreadsheet size={16} /> Export Terfilter
          </button>
        </div>
      </div>
          ...
          <div className="w-full max-w-sm bg-white rounded-[2.5rem] p-8 shadow-2xl border border-slate-100 animate-in zoom-in-95">
            <div className="flex items-center justify-between mb-6 pb-4 border-b border-slate-100">
               <div className="flex items-center gap-3">
                 ...
                 <div>
                   <h3 className="text-lg font-black text-slate-800">Export ke Excel/CSV</h3>
                 </div>
               </div>
            ...
            <div className="flex justify-end gap-3">
              ...
              <button onClick={executeExportNative} disabled={exporting} className="flex items-center gap-2 px-6 py-3 rounded-xl font-bold text-white bg-emerald-600 hover:bg-emerald-700 disabled:bg-emerald-400 shadow-lg shadow-emerald-500/30 text-xs transition-all">
                 {exporting ? <Loader2 size={16} className="animate-spin" /> : <Download size={16} />} Download .csv
              </button>
            </div>
          </div>
        </div>
