...
export default function Leads() {
  ...
  const executeExportXLSX = async () => {
    ...
    try {
      ...
      // Menggunakan dynamic import untuk modul XLSX via CDN agar terhindar dari build error esbuild
  ...
  const getSourceBadge = (source: string) => {
    ...
    if (baseSource === 'meta_ads') {
      ...
      styles = "bg-slate-800 text-slate-100 border-slate-700 shadow-md"; 
      label = "Direct / Random";
    } else {
              ...
              {loading ? (
                ...
                >
                  ...
                  <td className="px-6 py-5 align-middle">
                    ...
                    <div className="text-xs font-bold text-slate-500 font-mono mt-0.5 flex items-center gap-1.5">
                       {isLid ? (
                         <span className="bg-slate-200/60 text-slate-500 px-1.5 py-0.5 rounded text-[9px] uppercase tracking-widest font-black" title="ID Disembunyikan oleh Kebijakan Privasi Meta (LID)">Hidden ID</span>
                       ) : (
                         <span className="text-blue-600">+{lead.to_number}</span>
                       )}
                       {isLid && <span className="text-[9px] opacity-40">({lead.to_number})</span>}
                    </div>
