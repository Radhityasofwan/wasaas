        ...
        <div className="bg-white/40 backdrop-blur-3xl border border-white/80 rounded-[3rem] p-8 md:p-12 shadow-[0_20px_60px_rgba(31,38,135,0.05)] relative overflow-hidden">
          ...
          <div className="space-y-5 relative z-10">
            ...
            {loginMode === 'email' ? (
              ...
              <div className="space-y-4 animate-in fade-in duration-300">
                <div className="relative">
                  <label className="block text-[10px] font-black text-slate-400 mb-2 ml-4 uppercase tracking-[0.2em]">Alamat Email</label>
                  <input
                    type="email"
                    value={email}
                    ...
                    disabled={loading}
                    className="w-full px-6 py-4 rounded-[2rem] bg-white border border-slate-200 text-slate-700 placeholder-slate-300 focus:bg-white focus:ring-[6px] focus:ring-blue-500/10 focus:border-blue-400 outline-none transition-all duration-300 shadow-sm text-sm font-semibold"
                  />
                </div>
                <div className="relative">
                  <label className="block text-[10px] font-black text-slate-400 mb-2 ml-4 uppercase tracking-[0.2em]">Password</label>
                  <input
                    type="password"
                    value={password}
            ...
            <button
              onClick={handleLogin}
              disabled={loading}
              className={`group w-full py-4 mt-2 rounded-[2rem] text-white font-extrabold text-sm uppercase tracking-widest transition-all duration-500 relative overflow-hidden ${
                loading 
          ...
          {/* Toggle Menu: Email <-> API Key */}
          <div className="mt-8 pt-6 border-t border-slate-200 text-center relative z-10 flex flex-col items-center justify-center gap-2">
            <button 
              type="button"
              onClick={() => {
                setErr(null);
            ...
            >
              {loginMode === 'email' ? 'Gunakan API Key Akses' : 'Kembali menggunakan Email & Password'}
            </button>
