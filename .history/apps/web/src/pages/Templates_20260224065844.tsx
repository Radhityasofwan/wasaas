...
async function apiFetch<T>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  if (!res.ok) {
    // Menangani error object (misal dari Zod validation) agar tidak menjadi [object Object]
...
export default function Templates() {
  ...
  async function handleSave() {
    ...
    // Mencegah error Zod validation karena mengirim empty string pada media_url
  ...
  return (
    <div className="flex flex-col h-full bg-transparent overflow-hidden rounded-[2.5rem] relative">
      ...
      {/* HEADER */}
      <div className="flex items-center justify-between p-8 bg-white/40 backdrop-blur-3xl border-b border-white/20 shrink-0">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {templates.map(tpl => (
              <div key={tpl.id} className="bg-white/60 backdrop-blur-xl border border-white p-6 rounded-[2rem] shadow-sm hover:shadow-xl transition-all duration-300 group relative">
                ...
                <div className="bg-white/50 p-4 rounded-2xl border border-white/50 text-sm text-slate-600 font-medium line-clamp-3 leading-relaxed">
