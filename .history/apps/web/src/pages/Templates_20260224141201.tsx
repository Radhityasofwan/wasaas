...
import { 
  ...
  Trash2,
  X,
  Wand2, // Icon untuk Spintax/Magic
  UserCircle
} from "lucide-react";
...
export default function Templates() {
  ...
  // FEATURE: Insert Tag to Textarea Cursor Position
  const insertTag = (tag: string) => {
    ...
    if (textarea) {
      ...
      // Fallback jika ref tidak ditemukan
          ...
          <div className="w-full max-w-xl bg-white rounded-[3rem] shadow-2xl border border-white flex flex-col max-h-[90vh]">
            ...
            <div className="p-8 overflow-y-auto space-y-6 scrollbar-hide">
              ...
              <div>
                ...
                {/* NEW FEATURE: Personalisasi & Spintax UI */}
                <div className="mt-3 bg-indigo-50/50 border border-indigo-100 rounded-2xl p-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <button onClick={() => insertTag('{Halo|Hai|Selamat Pagi}')} className="px-3 py-1.5 bg-white border border-indigo-200 text-indigo-600 rounded-lg text-xs font-bold shadow-sm hover:bg-indigo-600 hover:text-white transition-colors">
                      Spintax: {Sapaan A | Sapaan B}
                    </button>
                  </div>
                  <p className="text-[10px] font-medium text-indigo-600/70 mt-3 leading-relaxed">
                    Spintax akan mengacak kata di dalam kurung kurawal secara otomatis saat pesan dikirim untuk menghindari deteksi spam WhatsApp.
                  </p>
                </div>
              </div>
