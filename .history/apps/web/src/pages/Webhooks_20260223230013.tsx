...
export default function Webhooks() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (res.data) {
          setUrl(res.data.url); setIsActive(res.data.is_active);
          setSelected(res.data.events || []); setSecret(res.data.secret_head);
        } else {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      await apiFetch("webhooks/set", { method: "POST", body: JSON.stringify({ url, status: isActive ? "active" : "inactive", events: selected }) });
      alert("Tersimpan!");
    } catch { alert("Gagal simpan"); } finally { setSaving(false); }
  };
        ...
        <form onSubmit={handleSave} className="space-y-10">
          ...
          <div className="flex flex-col md:flex-row justify-between items-center pt-10 border-t border-white/40 gap-8">
             <div className="space-y-2">
                ...
                <code className="bg-slate-100/50 px-6 py-3 rounded-xl border border-white text-xs font-mono text-slate-600 italic">{secret ? `${secret}********` : "Belum aktif"}</code>
             </div>
