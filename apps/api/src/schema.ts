...
CREATE TABLE IF NOT EXISTS message_templates (
  ...
  media_name VARCHAR(255) NULL,
  media_url TEXT NULL,
  category ENUM('broadcast', 'follow_up', 'general') NOT NULL DEFAULT 'general',
...
/* =========================================================
   NEW: MASTER CRM LEADS (Entitas Utama Pengelolaan Prospek)
   ========================================================= */
...
CREATE TABLE IF NOT EXISTS crm_leads (
  ...
  /* Pelacakan Trafik / Sumber Leads */
  source VARCHAR(80) NOT NULL DEFAULT 'manual', /* meta_ads, web, ig, tiktok, manual, broadcast_reply */
  
  /* Klasifikasi Suhu Prospek */
  status ENUM('cold','warm','hot','converted','dead') NOT NULL DEFAULT 'cold',
  
  /* JSON untuk menyimpan Custom Tags (Label Warna) dari Inbox UI */
  tags_json JSON NULL,
  
  last_interacted_at DATETIME NULL,
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at DATETIME NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP,

  PRIMARY KEY (id),
  UNIQUE KEY uq_lead_phone (tenant_id, phone_number),
