...
export default function Dashboard() {
  ...
  // --- CONFIGURATION CHARTS (GOOGLE VIBES) ---
  ...
  const barData = {
    labels: Object.keys(stats.sourceData).length > 0 ? Object.keys(stats.sourceData).map(s => s.toUpperCase()) : ['META', 'IG', 'WEB', 'DIRECT'],
    datasets: [{
      label: 'Volume Prospek',
      data: Object.values(stats.sourceData).length > 0 ? Object.values(stats.sourceData) : [45, 32, 12, 28],
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-32 text-slate-400 gap-4">
        ...
        <p className="font-bold tracking-widest uppercase text-[10px]">Mengkonsolidasi Data...</p>
      </div>
    );
  }
    ...
    <div className="max-w-7xl mx-auto space-y-6 md:space-y-8 animate-in fade-in duration-500 pb-20">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        ...
        <div className="bg-white border border-slate-100 p-6 rounded-3xl shadow-sm hover:shadow-md transition-all group">
          ...
          <div className="mt-4 flex items-center gap-1.5 text-[10px] font-bold text-rose-600">
        ...
        <div className="bg-white border border-slate-100 p-6 rounded-3xl shadow-sm hover:shadow-md transition-all group">
          ...
          <div className="mt-4 text-[10px] font-bold text-slate-500">
            Tingkat Gagal: <span className="text-rose-500">{((stats.broadcastFailed / (stats.broadcastSent || 1)) * 100).toFixed(1)}%</span>
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-100 p-6 rounded-3xl shadow-sm hover:shadow-md transition-all group">
          ...
          <div className="flex items-baseline gap-2">
            <span className="text-3xl font-bold text-slate-800">{stats.followupsActive + (stats.autoReplyActive > 0 ? 1 : 0)}</span>
          ...
          <div className="mt-4 flex items-center gap-1.5 text-[10px] font-bold text-indigo-600">
            <Bot size={14} /> {stats.autoReplyActive} Aturan Bot Aktif
          </div>
        </div>
